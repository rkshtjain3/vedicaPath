...
} from './shadbala-jhora-benchmark-schema.js';

export class ShadbalaBenchmarkRunner {
  private engine: SwissEphemerisEngine;
  private tolerancePolicy: ShadbalaTolerancePolicy;

  constructor(tolerancePolicy: ShadbalaTolerancePolicy = DEFAULT_SHADBALA_TOLERANCE_POLICY) {
    this.engine = new SwissEphemerisEngine();
    this.tolerancePolicy = tolerancePolicy;
  }

  public async runBenchmarkCase(benchmarkCase: ShadbalaJHoraBenchmark): Promise<BenchmarkCaseResult> {
    ...
    const componentResults: ComponentComparisonResult[] = [];
    ...
    for (const [planetName, refValues] of Object.entries(expected)) {
      ...
      if (!planetShadbala || !refValues) continue;

      const comparisons: Array<{ component: string; actual: number | undefined; expected: number | undefined }> = [
      ...
      for (const comp of comparisons) {
        if (comp.expected === undefined) continue;
