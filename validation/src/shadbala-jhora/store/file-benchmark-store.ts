...
export function computeReferenceHash(referenceValues: Record<string, Record<string, number>>): string {
...
export class FileBenchmarkStore implements BenchmarkStore {
  private data: Record<string, StoredBenchmarkData> = {};
  private engine: SwissEphemerisEngine;
  private runner: ShadbalaBenchmarkRunner;
  ...
  public async getCases(): Promise<BenchmarkCaseSummary[]> {
    ...
    for (const c of SHADBALA_JHORA_BENCHMARK_CASES) {
      ...
      if (stored && stored.referenceValues && Object.keys(stored.referenceValues).length > 0) {
        const evalCase = {
          ...
          validationStatus: 'PARTIALLY_VALIDATED' as const,
        };
  ...
  public async getCase(caseId: string): Promise<any | null> {
  ...
  public async saveReference(input: SaveBenchmarkReferenceInput): Promise<StoredBenchmarkData> {
    ...
    const record: StoredBenchmarkData = {
      caseId,
  ...
  public async getResults(caseId: string): Promise<any | null> {
    const caseDef = SHADBALA_JHORA_BENCHMARK_CASES.find((c) => c.id === caseId);
    if (!caseDef) return null;
  ...
  public async exportBenchmark(): Promise<FullBenchmarkExport> {
    return {
      exportVersion: '1.0.0',
  ...
  public async importBenchmark(exportObj: FullBenchmarkExport): Promise<{ importedCount: number }> {
