import { BenchmarkCase, VerificationStatus, calculateReferenceCompleteness } from '@vedica/benchmark-store';
...
export function auditBenchmarkCases(casesInput?: BenchmarkCase[]): BenchmarkAuditResult {
  ...
  const auditRecords: BenchmarkCaseAuditRecord[] = [];
  ...
  for (const c of cases) {
    ...
    const vStatus: VerificationStatus = c.referenceSource?.verificationStatus || 'UNVERIFIED';

    if (hasRefData) casesWithReferenceData++;
    ...
    const refAvail: BenchmarkCaseAuditRecord['referenceAvailability'] =
    ...
    const auditRec: BenchmarkCaseAuditRecord = {
      caseId: c.id,
      title: c.title || c.id,
  ...
  const summary: BenchmarkRegistrySummary = {
