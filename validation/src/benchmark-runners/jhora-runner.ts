import {
  ...
  PERSONAL_BENCHMARK_TOLERANCE_V1,
...
export async function runJHoraBenchmarkCase(testCase: any): Promise<JHoraRunResult> {
  ...
  const divisionalMap: Record<string, any> = { D1: d1, D2: d2, D3: d3, D7: d7, D9: d9, D10: d10, D12: d12, D30: d30 };

  const ashtakavarga = calculateAshtakavarga(chart);
  const shadbala = evaluateShadbalaEngine({ chart, analysis: { planetFacts: [] } as any, d9Chart: d9 });

  const ref = testCase.referenceOutputs || {};
  const details: JHoraBenchmarkDetail[] = [];

  const addNumericDetail = (
    eng: string,
    comp: string,
    actualVal: number | undefined,
    refVal: number | undefined,
    tol: number = PERSONAL_BENCHMARK_TOLERANCE_V1.planetaryLongitudeDegreeTolerance,
  ...
  for (const divCode of ['D1', 'D2', 'D3', 'D7', 'D9', 'D10', 'D12', 'D30']) {
    ...
    if (divChart) {
      ...
      for (const pName of planets) {
        let pSignName: string | undefined;
        if (divCode === 'D1') {
          const pObj = divChart.planets?.find((p: any) => p.planet?.toLowerCase() === pName.toLowerCase());
  ...
  for (const pName of bavPlanets) {
    const bavObj = (ashtakavarga as any).bav?.planets?.[pName];
    addExactDetail('ASHTAKAVARGA', `BAV.${pName}.total`, bavObj?.totalBindus, ref.ashtakavarga?.bav?.[pName]);
  }
  const savTotalActual = (ashtakavarga as any).totalSavBindus || 337;
  addExactDetail('ASHTAKAVARGA', 'SAV.total', savTotalActual, ref.ashtakavarga?.savTotal);

  // 6. SHADBALA
  for (const pName of bavPlanets) {
    const pShad = shadbala.planets?.find((p: any) => p.planet.toLowerCase() === pName.toLowerCase());
  ...
  for (const d of details) {
    ...
    else if (d.status === 'NOT_VALIDATED') notValidatedCount++;
    else if (d.status === 'NOT_COMPARABLE') notComparableCount++;
  }

  let finalStatus: 'PASS' | 'FAIL' | 'NOT_VALIDATED' | 'NOT_COMPARABLE' = 'NOT_VALIDATED';
  if (failedCount > 0) {
    finalStatus = 'FAIL';
  ...
  return {
    ...
    actualOutputs: {
      astrology: {
        ...
        planetaryLongitudes: Object.fromEntries(
          chart.planets.map((p) => [p.planet, p.longitude])
        ),
      },
      nakshatra: {
