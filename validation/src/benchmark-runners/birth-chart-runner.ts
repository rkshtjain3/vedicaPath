...
export function computeCircularAngularDifference(deg1: number, deg2: number): number {
...
export function calculateRMS(numbers: number[]): number {
  if (numbers.length === 0) return 0;
  const squareSum = numbers.reduce((sum, val) => sum + val * val, 0);
  return Math.sqrt(squareSum / numbers.length);
}

export async function runBirthChartBenchmark(
  testCase: BenchmarkCase,
  tolerance = 0.05,
  targetSoftware?: string
): Promise<BirthChartRunResult> {
  ...
  if (targetSoftware && testCase.multiReferences && testCase.multiReferences.length > 0) {
    const matched = testCase.multiReferences.find(
      (m) => m.sourceSoftware.toLowerCase() === targetSoftware.toLowerCase()
    );
    if (matched) {
  ...
  const verificationStatus: VerificationStatus = refSource?.verificationStatus || 'UNVERIFIED';
  ...
  let configurationMismatchNote: string | undefined;
  ...
  const ref = (refValues || {}) as Record<string, any>;
  const details: BirthChartBenchmarkDetail[] = [];
  const planetaryAngularDiffs: number[] = [];

  // Helper for angular comparison
  const evaluateAngular = (
    cat: BirthChartBenchmarkDetail['category'],
    key: string,
    field: string,
    actualVal: number | undefined,
    ...
    const item: BirthChartBenchmarkDetail = {
      category: cat,
      planetOrKey: key,
      field,
  ...
  const divMatrix: ComponentAccuracyMetrics['divisional'] = {};
    ...
    if (divChart && targetRef) {
      for (const pName of planetNames) {
        const divP = divCode === 'D1'
          ? divChart.planets?.find((p: any) => p.planet?.toLowerCase() === pName.toLowerCase())
        ...
        if (expectedSign !== undefined && expectedSign !== null) {
          ...
          const detailItem: BirthChartBenchmarkDetail = {
            category: 'DIVISIONAL',
  ...
  for (const d of details) {
    if (d.status === 'PASS') passedCount++;
    else if (d.status === 'FAIL') failedCount++;
    else if (d.status === 'NOT_VALIDATED') unvalidatedCount++;
  }

  let overallResult: BirthChartRunResult['overallResult'] = 'NOT_VALIDATED';
  let validationExecutionResult: ValidationExecutionResult = 'NOT_RUN';
  ...
  const meanAngularDiff =
    planetaryAngularDiffs.length > 0
  ...
  const planetaryPassPercentage =
    planetLongDetails.length > 0 ? Math.round((planetLongPassed / planetLongDetails.length) * 100) : 0;
  ...
  const componentBreakdown: ComponentAccuracyMetrics = {
    planetary: {
  ...
  return {
    ...
    actualChartOutputs: {
      ...
      divisionalCharts: Object.fromEntries(
        Object.entries(divisionalCharts).map(([k, v]) => [k, { lagna: v.ascendant || v.lagna, planets: v.planets }])
      ),
    },
...
export async function runMultiCaseChartBenchmark(
  cases: BenchmarkCase[],
  tolerance = 0.05,
  targetSoftware?: string
): Promise<MultiCaseBenchmarkSummary> {
  const caseResults: BirthChartRunResult[] = [];
  ...
  const allObservedAngularDiffs: number[] = [];
  ...
  for (const c of cases) {
    ...
    if (res.validationExecutionResult === 'PASS') passedCases++;
    else if (res.validationExecutionResult === 'FAIL') failedCases++;
    else if (res.validationExecutionResult === 'INCOMPARABLE_CONFIGURATION') incomparableCases++;

    for (const d of res.details) {
      if (d.status !== 'NOT_VALIDATED') {
        totalComparedComponents++;
        if (d.passed) totalPassedComponents++;
  ...
  const meanDiff =
    allObservedAngularDiffs.length > 0
  ...
  const overallComponentPassRate =
    totalComparedComponents > 0 ? Math.round((totalPassedComponents / totalComparedComponents) * 100) : 0;
