...
export function runShadbalaBenchmark(
  ...
  const compareSection = (sectionRef: Record<string, any> | undefined, sectionResult: any, sectionName: string) => {
    if (!sectionRef || !sectionResult) return;
  ...
  const extractComponent = (componentName: 'sthana' | 'dig' | 'naisargika' | 'cheshta') => {
