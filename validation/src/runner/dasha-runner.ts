...
export function runDashaBenchmark(testCase: DashaBenchmarkCase) {
  ...
  const passed =
    current.mahadasha?.lord === testCase.expected.currentMahadasha &&
...
export function verifyDashaBoundaryContinuity(moonLongitude: number, birthInstant: Date) {
  const result = generateMahadashas({
    birthInstant,
    moonLongitude,
  });

  const checks: Array<{ description: string; passed: boolean; details?: string }> = [];
