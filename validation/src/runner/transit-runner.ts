...
export async function runTransitBenchmark(testCase: TransitBenchmarkCase, tolerance: number = 0.05) {
  ...
  const details: Array<{ planet: string; expected: number; actual: number; diff: number; passed: boolean }> = [];
...
export async function verifyTransitReproducibility(testInstant: string): Promise<boolean> {
  const engine = new SwissEphemerisEngine();
