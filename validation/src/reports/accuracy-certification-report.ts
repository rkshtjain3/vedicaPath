...
import {
  ...
  calculateMedian,
  calculateRMS,
} from '../benchmark-runners/birth-chart-runner.js';
...
export async function generateAccuracyCertification(
  benchmarkCases: BenchmarkCase[],
  tolerance = 0.05,
  targetSoftware?: string
): Promise<{ summary: CertificationSummary; textReport: string }> {
  const resultsByCase: BirthChartRunResult[] = [];
  ...
  for (const c of benchmarkCases) {
    ...
    for (const d of res.details) {
      if (d.status !== 'NOT_VALIDATED') {
        ...
        if (d.passed) totalPassedComparisons++;
        else totalFailedComparisons++;
  ...
  for (const r of resultsByCase) {
    ...
    lines.push(
      `[${r.validationExecutionResult.padEnd(26)}] ${r.benchmarkId}: ${r.caseName} (Completeness: ${r.completeness.overallPercent}%${vStr})`
    );
  }
  lines.push('');
