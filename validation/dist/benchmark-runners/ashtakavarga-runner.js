export function runAshtakavargaBenchmark(testCase, ashtakavargaResult) {
    const results = [];
    ...
    if (ref.bav) {
        for (const [planetName, expectedBav] of Object.entries(ref.bav)) {
            ...
            if (actualBav) {
                if (expectedBav.total !== undefined) {
                    results.push({
                        planetOrKey: planetName,
                        field: 'BAV Total',
                        expectedValue: expectedBav.total,
                        actualValue: actualBav.totalBindus,
                        passed: actualBav.totalBindus === expectedBav.total
                    });
                }
            }
        }
...
//# sourceMappingURL=ashtakavarga-runner.js.map