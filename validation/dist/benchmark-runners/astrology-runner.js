export function isWithinAngularTolerance(expected, actual, tolerance = 0.05) {
    const diff = Math.abs(expected - actual);
    const circularDiff = Math.min(diff, 360 - diff);
    return circularDiff <= tolerance;
}
export function runAstrologyBenchmark(testCase, chart, tolerance = 0.05) {
    const results = [];
    ...
    if (ref.planetaryLongitudes) {
        for (const [planetName, expectedLngRaw] of Object.entries(ref.planetaryLongitudes)) {
            const expectedLng = expectedLngRaw;
    ...
    if (ref.nakshatras) {
        for (const [planetName, expectedNakRaw] of Object.entries(ref.nakshatras)) {
            const expectedNak = expectedNakRaw;
            if (planetName === 'Lagna') {
...
//# sourceMappingURL=astrology-runner.js.map