...
export function assessBenchmarkSuiteQuality(casesInput) {
    ...
    const verifiedCount = cases.filter((c) => c.referenceSource?.verificationStatus === 'VERIFIED').length;
    ...
    if (verifiedCount < total) {
        limitations.push(`${total - verifiedCount} benchmark cases remain unverified or lack complete third-party reference data.`);
    }
    ...
    const boundaryCount = cases.filter((c) => c.id.includes('006') || c.id.includes('007') || c.category.includes('BOUNDARY')).length;
    ...
    const softwareSources = new Set(cases
    ...
    limitations.push('Benchmark suite quality score reflects test program maturity and reference coverage ONLY; it does not measure astrological accuracy or predictive validity.');
    return {
        profileVersion: 'BENCHMARK_QUALITY_V1',
        evaluatedAt: new Date().toISOString(),
        qualityScore,
...
//# sourceMappingURL=benchmark-suite-quality.js.map