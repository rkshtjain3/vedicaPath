export async function validateBenchmark(benchmark, engine, toleranceDegree = 0.05) {
    const items = [];
    ...
    if (benchmark.expected.ascendant) {
        ...
        const calcAscSignName = typeof birthChart.lagna.sign === 'string'
            ? birthChart.lagna.sign
            : birthChart.lagna.sign.name;
        ...
        if (!signPassed || !degPassed)
            overallPassed = false;
    }
    // Validate Planets
    if (benchmark.expected.planets) {
        for (const [planetName, expPlanet] of Object.entries(benchmark.expected.planets)) {
            const calcPlanet = birthChart.planets.find((p) => p.planet.toLowerCase() === planetName.toLowerCase());
            if (!calcPlanet) {
                items.push({
            ...
            const calcPlanetSignName = typeof calcPlanet.sign === 'string'
                ? calcPlanet.sign
                : calcPlanet.sign.name;
            // 1. Sign
            ...
            if (expPlanet.nakshatra) {
                const nakName = typeof calcPlanet.nakshatra === 'string'
                    ? calcPlanet.nakshatra
                    : calcPlanet.nakshatra.nakshatra || calcPlanet.nakshatra.name;
            ...
            if (expPlanet.pada !== undefined) {
                const padaVal = calcPlanet.nakshatra.pada;
    ...
    const finalStatus = overallPassed ? 'PASS' : 'FAIL';
    return {
        benchmarkId: benchmark.id,
        description: benchmark.description,
