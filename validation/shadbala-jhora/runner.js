...
export class ShadbalaBenchmarkRunner {
    ...
    constructor(tolerancePolicy = DEFAULT_SHADBALA_TOLERANCE_POLICY) {
        this.engine = new SwissEphemerisEngine(PERSONAL_VEDIC_V1);
        this.tolerancePolicy = tolerancePolicy;
    }
    async runBenchmarkCase(benchmarkCase) {
        const { birthDetails, expected } = benchmarkCase;
        // If expected values map is empty or status is NOT_VALIDATED, return early with NOT_VALIDATED status
        ...
        // 1. Calculate Birth Chart using Swiss Ephemeris Engine
        const chartInput = {
            birthTime: {
        ...
        // 2. Calculate Shadbala Engine output
        const shadbalaResult = evaluateShadbalaEngine({ chart, analysis, d9Chart }, PERSONAL_SHADBALA_V1);
        const componentResults = [];
        let hasFailures = false;
        // 3. Compare each planet & component against reference expected values
