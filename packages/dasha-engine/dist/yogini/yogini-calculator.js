import { NAKSHATRA_NAMES, } from '../vimshottari/canonical-vimshottari.js';
import { MS_PER_JULIAN_YEAR } from '../vimshottari/dasha-calculator.js';
import { TOTAL_YOGINI_YEARS, YOGINI_CYCLE, getStartingYoginiFromNakshatra, } from './canonical-yogini.js';
/**
 * Calculates starting Yogini Dasha balance at birth from Moon longitude.
 */
export function calculateBirthYoginiBalance(moonLongitude) {
 ...
 */
export function getYoginiIndex(yogini) {
 ...
 */
export function generateYoginiAntardashas(mahadashaYogini, fullMahadashaYears, mahadashaStart, mahadashaEnd) {
    const antardashas = [];
    ...
    for (let i = 0; i < 8; i++) {
        ...
        const durationYears = (fullMahadashaYears * antarConfig.years) / TOTAL_YOGINI_YEARS;
        ...
        if (periodEndMs > mahadashaStart.getTime() &&
            ...
            const actualDurationYears = (actualEnd.getTime() - actualStart.getTime()) / MS_PER_JULIAN_YEAR;
            antardashas.push({
                level: 'ANTARDASHA',
...
export function generateYoginiDashas(input) {
    ...
    for (let i = 0; i < totalPeriods; i++) {
        ...
        const children = generateYoginiAntardashas(yoginiConfig, fullYears, start, end);
        mahadashas.push({
            level: 'MAHADASHA',
...
//# sourceMappingURL=yogini-calculator.js.map