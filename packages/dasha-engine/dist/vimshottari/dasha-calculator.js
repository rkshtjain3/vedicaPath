import { NAKSHATRA_LORDS, NAKSHATRA_NAMES, TOTAL_VIMSHOTTARI_YEARS, VIMSHOTTARI_CYCLE, } from './canonical-vimshottari.js';
 ...
 */
export function calculateBirthDashaBalance(moonLongitude) {
    const normalizedMoon = (moonLongitude % 360 + 360) % 360;
    const nakshatraSpan = 360 / 27; // 13.333333333333334 degrees
    const nakshatraIndex = Math.min(26, Math.floor(normalizedMoon / nakshatraSpan));
 ...
 */
export function getLordIndex(lord) {
    return VIMSHOTTARI_CYCLE.findIndex((c) => c.lord === lord);
}
/**
 * Generates Pratyantardashas for an Antardasha period.
 */
export function generatePratyantardashas(antardashaLord, antardashaDurationYears, antardashaStart) {
 ...
 */
export function generateAntardashas(mahadashaLord, fullMahadashaYears, mahadashaStart, mahadashaEnd) {
    const antardashas = [];
    ...
    for (let i = 0; i < 9; i++) {
        ...
        if (periodEndMs > mahadashaStart.getTime() && periodStartMs < mahadashaEnd.getTime()) {
            ...
            const children = generatePratyantardashas(lordConfig.lord, durationYears, new Date(periodStartMs)).filter((p) => p.end.getTime() > mahadashaStart.getTime() && p.start.getTime() < mahadashaEnd.getTime());
            antardashas.push({
                level: 'ANTARDASHA',
                lord: lordConfig.lord,
                start: actualStart,
...
export function getCurrentDasha(input) {
    ...
    const activeMaha = input.mahadashas.find((m) => targetTime >= m.start.getTime() && targetTime < m.end.getTime());
    if (!activeMaha) {
        return { instant: input.instant };
    }
    const activeAntar = activeMaha.children?.find((a) => targetTime >= a.start.getTime() && targetTime < a.end.getTime());
    const activePrat = activeAntar?.children?.find((p) => targetTime >= p.start.getTime() && targetTime < p.end.getTime());
    return {
...
//# sourceMappingURL=dasha-calculator.js.map