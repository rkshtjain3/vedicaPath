...
export function resolveTimezoneByCountryAndCoords(
  countryCode: string,
  latitude: number,
  longitude: number
): string {
  ...
  if (code === 'CA') {
    if (longitude < -115) return 'America/Vancouver';
    if (longitude < -100) return 'America/Edmonton';
    if (longitude < -85) return 'America/Winnipeg';
    return 'America/Toronto';
  }

  // Australia Regional Coordinate Bounding Box Resolution
  if (code === 'AU') {
    if (longitude < 125) return 'Australia/Perth';
    if (longitude < 140) return 'Australia/Adelaide';
