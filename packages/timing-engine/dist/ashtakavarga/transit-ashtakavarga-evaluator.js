...
import { PERSONAL_TRANSIT_ASHTAKAVARGA_V1 } from './profile.js';
export async function evaluateTransitAshtakavarga(input) {
    ...
    for (const planetName of profile.supportedPlanets) {
        ...
        const pos = transitPositions.find((p) => p.planet.toUpperCase() === planetKey);
        if (!pos)
            continue;
        ...
        const evidenceStrings = buildTransitAshtakavargaEvidenceStrings(planetKey, transitIsoDate, transitSign, transitHouseFromLagna, bavEval, savEval);
...
//# sourceMappingURL=transit-ashtakavarga-evaluator.js.map