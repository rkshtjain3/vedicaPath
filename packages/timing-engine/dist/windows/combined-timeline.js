...
import { getDomainTransitTimeline, calculateTransitTimelineSteps, } from './transit-timeline.js';
...
function getCombinedActivityRating(score, profile) {
    if (score >= profile.scoringBoundaries.combinedActivity.HIGH_MIN)
        return 'HIGH';
    if (score >= profile.scoringBoundaries.combinedActivity.MODERATE_MIN)
        return 'MODERATE';
    return 'LOW';
}
function getRatingFromScore(score, highMin, modMin) {
    if (score >= highMin)
        return 'HIGH';
    if (score >= modMin)
        return 'MODERATE';
    ...
    for (const dPeriod of parsedDashaPeriods) {
        for (const tWindow of parsedTransitWindows) {
            ...
            if (overlapStartMs < overlapEndMs) {
                ...
                const dashaRating = getRatingFromScore(dashaScore, profile.scoringBoundaries.dashaActivation.HIGH_MIN, profile.scoringBoundaries.dashaActivation.MODERATE_MIN);
                const transitRating = getRatingFromScore(transitScore, profile.scoringBoundaries.transitActivation.HIGH_MIN, profile.scoringBoundaries.transitActivation.MODERATE_MIN);
                const factors = [];
                if (dPeriod.dashaActivation.mahadasha.connected) {
                    factors.push(`Mahadasha lord (${dPeriod.dashaActivation.mahadasha.lord}) connected to ${domain}`);
                }
                if (dPeriod.dashaActivation.antardasha.connected) {
                    factors.push(`Antardasha lord (${dPeriod.dashaActivation.antardasha.lord}) connected to ${domain}`);
                }
                if (dPeriod.dashaActivation.pratyantardasha.connected) {
                    factors.push(`Pratyantardasha lord (${dPeriod.dashaActivation.pratyantardasha.lord}) connected to ${domain}`);
                }
...
export async function evaluateTimingEngine(params) {
    ...
    const domains = ['CAREER', 'WEALTH', 'RELATIONSHIPS', 'PROPERTY'];
    const currentStatus = {};
    const timelines = {};
...
//# sourceMappingURL=combined-timeline.js.map