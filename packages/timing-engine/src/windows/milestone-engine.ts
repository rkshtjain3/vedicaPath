...
export function calculateLifeMilestones(params: {
  ...
  const propertyWindows: LifeMilestoneWindow[] = [];
  const marriageWindows: LifeMilestoneWindow[] = [];
  const careerWindows: LifeMilestoneWindow[] = [];
  const relocationWindows: LifeMilestoneWindow[] = [];
  ...
  for (const m of mahadashas) {
    const mahaLord = m.lord || (m as any).planet || (m as any).name || 'Jupiter';
    const subPeriods = m.children || (m as any).antardashas || [];

    if (subPeriods.length > 0) {
      for (const a of subPeriods) {
        const antarLord = a.lord || (a as any).planet || (a as any).name || 'Mars';
        const startRaw = a.start || (a as any).startDate;
        const endRaw = a.end || (a as any).endDate;
      ...
      const startRaw = m.start || (m as any).startDate;
      const endRaw = m.end || (m as any).endDate;
      const start = startRaw instanceof Date ? startRaw : new Date(startRaw);
      const end = endRaw instanceof Date ? endRaw : new Date(endRaw);
      relevantPeriods.push({
        maha: mahaLord,
