...
import {
  classifyTenGod,
} from '../analysis/ten-gods-calculator.js';
import {
  AnnualPillarInfo,
  DaYunDetails,
  FourPillars,
...
export function calculateLuckPillars(
  ...
  if (startingAge < 1) startingAge = 1;
  if (startingAge > 10) startingAge = 10;
...
export function calculateAnnualPillar(
  fourPillars: FourPillars,
  ...
  const clashesWithNatal: string[] = [];
  const harmoniesWithNatal: string[] = [];
