export function analyzeDayMasterAndElements(fourPillars) {
    ...
    else if ((seasonElement === 'Wood' && dmElement === 'Fire') ||
    ...
    else if ((dmElement === 'Wood' && seasonElement === 'Fire') ||
    ...
    else if ((seasonElement === 'Wood' && dmElement === 'Earth') ||
    ...
    const visibleMap = { Wood: 0, Fire: 0, Earth: 0, Metal: 0, Water: 0 };
    const hiddenWeightMap = { Wood: 0, Fire: 0, Earth: 0, Metal: 0, Water: 0 };
    const pillarsList = [
        fourPillars.year,
        fourPillars.month,
        fourPillars.day,
    ...
    const fiveElements = elements.map((elem) => {
        const vis = visibleMap[elem];
        const weight = hiddenWeightMap[elem];
        let status = 'BALANCED';
        if (vis >= 3 || weight >= 150)
            status = 'DOMINANT';
        else if (vis >= 2 || weight >= 90)
            status = 'STRONG';
        else if (weight >= 40)
            status = 'BALANCED';
        else if (weight > 0)
    ...
    if (seasonalStatus === 'IN_SEASON')
        score += 40;
    else if (seasonalStatus === 'PROSPEROUS')
        score += 30;
    else if (seasonalStatus === 'WEAKENING')
        score += 10;
    else if (seasonalStatus === 'TRAPPED')
        score -= 10;
    else if (seasonalStatus === 'DEAD')
        score -= 20;
    // Add support from same element (Peers) and Resource elements
    const sameElemInfo = fiveElements.find((e) => e.element === dmElement);
    if (sameElemInfo)
        score += sameElemInfo.hiddenWeightPercentage * 0.15;
    score = Math.max(5, Math.min(95, Math.round(score)));
    let classification = 'BALANCED';
    if (score >= 80)
        classification = 'EXTREMELY_STRONG';
    else if (score >= 60)
