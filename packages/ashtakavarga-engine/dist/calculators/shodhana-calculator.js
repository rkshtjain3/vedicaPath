 ...
 */
export function performTrikonaShodhana(signPoints) {
    const result = { ...signPoints };
 ...
 */
export function getOccupiedSigns(chart) {
    const map = {};
 ...
 */
export function performEkadhipatyaShodhana(trikonaPoints, chart) {
    const result = { ...trikonaPoints };
    const explanation = [];
    ...
    for (const pair of DUAL_LORDSHIP_PAIRS) {
        ...
        if (occ1 && occ2) {
            ...
            explanation.push(`${pair.planet} (${s1} & ${s2}): Both signs occupied by planets. No Ekadhipatya reduction.`);
        }
        else if (occ1 && !occ2) {
            // Rule 2a: S1 occupied, S2 vacant
            if (b2 > b1) {
                result[s2] = b1;
                explanation.push(`${pair.planet}: ${s1} occupied (${b1}b), ${s2} vacant (${b2}b -> reduced to ${b1}b).`);
            }
            else {
                result[s2] = 0;
                explanation.push(`${pair.planet}: ${s1} occupied (${b1}b), ${s2} vacant (${b2}b -> reduced to 0).`);
            }
        }
        else if (!occ1 && occ2) {
            ...
            if (b1 > b2) {
                result[s1] = b2;
                explanation.push(`${pair.planet}: ${s2} occupied (${b2}b), ${s1} vacant (${b1}b -> reduced to ${b2}b).`);
            }
            else {
                result[s1] = 0;
                explanation.push(`${pair.planet}: ${s2} occupied (${b2}b), ${s1} vacant (${b1}b -> reduced to 0).`);
            }
        }
        else {
            // Rule 3: Both vacant
            if (b1 === b2) {
                result[s1] = 0;
                result[s2] = 0;
                explanation.push(`${pair.planet}: Both ${s1} and ${s2} vacant with equal bindus (${b1}b -> both become 0).`);
            }
            else {
                if (b1 === 0 || b2 === 0) {
                    // If one is already 0, non-zero remains unchanged
                    explanation.push(`${pair.planet}: Both ${s1} and ${s2} vacant; one sign has 0 bindus. Retaining non-zero value.`);
                }
                else {
                    ...
                    result[s1] = minB;
                    result[s2] = minB;
                    explanation.push(`${pair.planet}: Both ${s1} and ${s2} vacant with unequal bindus (${b1}b vs ${b2}b -> larger reduced to ${minB}b).`);
                }
            }
        }
    }
 ...
 */
export function calculatePindas(ekadhipatyaPoints, chart) {
    // 1. Rashi Pinda
    let rashiPinda = 0;
    for (const rashi of RASHIS) {
        const bindus = ekadhipatyaPoints[rashi.name] ?? 0;
        const mult = RASHI_GUNAKARA[rashi.name] ?? 7;
        rashiPinda += bindus * mult;
    }
    // 2. Graha Pinda
    let grahaPinda = 0;
    for (const p of chart.planets) {
        const planetKey = p.planet.toUpperCase();
 ...
 */
export function performFullShodhana(bav, chart) {
    const trikonaShodhana = {};
    const ekadhipatyaShodhana = {};
    const planetaryPindas = {};
    const savTrikona = {};
    const savEkadhipatya = {};
    for (const rashi of RASHIS) {
    ...
    for (const planet of ASHTAKAVARGA_PLANETS) {
        ...
        const { ekadhipatyaPoints, explanation } = performEkadhipatyaShodhana(trikona, chart);
