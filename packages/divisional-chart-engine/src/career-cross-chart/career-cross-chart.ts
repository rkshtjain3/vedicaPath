 ...
 */
export function calculateCareerCrossChartFacts(
  birthChart: BirthChart,
  d10Chart: DivisionalChart,
  d1Analysis: ChartAnalysisResult,
  d9Chart?: DivisionalChart,
  strengthScores?: Record<string, { totalScore: number; classification: string }>
): CareerCrossChartAnalysis {
  ...
  const getHouseSign = (lagnaId: number, houseNum: number) => {
    const signId = ((lagnaId + houseNum - 2) % 12) + 1;
    return RASHIS[signId - 1];
  };

  // Helper: Get planet placement and dignity in D1
  const getD1PlanetFact = (planet: PlanetName): CareerIndicatorFact => {
  ...
  const getD10PlanetFact = (planet: PlanetName): CareerIndicatorFact => {
  ...
  const candidatePlanets: PlanetName[] = ['Sun', 'Saturn', 'Jupiter'];
  if (!candidatePlanets.includes(d1TenthLordName)) candidatePlanets.push(d1TenthLordName);
  if (!candidatePlanets.includes(d10TenthLordName)) candidatePlanets.push(d10TenthLordName);

  const crossChartPlanets: CareerCrossChartItem[] = candidatePlanets.map((planet) => {
