 ...
 */
export function getSignCategory(rashiId: number): SignCategory {
  const normId = ((rashiId - 1) % 12) + 1;
  if ([1, 4, 7, 10].includes(normId)) return 'MOVABLE';
  if ([2, 5, 8, 11].includes(normId)) return 'FIXED';
  return 'DUAL';
}
 ...
 */
export function getNavamsaStartSign(rashiId: number): number {
 ...
 */
export function calculateNavamsaPosition(sourceLongitude: number): DivisionalPosition {
