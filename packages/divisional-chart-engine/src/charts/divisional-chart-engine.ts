import { BirthChart, PlanetName, RASHIS } from '@vedica/astrology-core';
import { calculatePlanetDignity, ChartAnalysisResult, PlanetFact } from '@vedica/analysis-engine';
...
import {
  ...
  DivisionalChartType,
  DivisionalHouse,
  DivisionalPlanetAnalysis,
  DivisionalPosition,
  VargaComparisonItem,
  VargaComparisonResult,
  VargaDashamsaComparisonItem,
  VargaDashamsaComparisonResult,
} from '../types/divisional-types.js';
...
export const DIVISIONAL_PROFILES: Record<DivisionalChartType, DivisionalChartProfile> = {
  D1: {
    type: 'D1',
    division: 1,
    name: 'Rashi',
...
export function calculateDivisionalChart(
  ...
  const calcPosition = (longitude: number): DivisionalPosition => {
    if (type === 'D1') {
    ...
    if (type === 'D2') return calculateHoraPosition(longitude);
    if (type === 'D3') return calculateDrekkanaPosition(longitude);
    if (type === 'D4') return calculateChaturthamsaPosition(longitude);
    if (type === 'D7') return calculateSaptamsaPosition(longitude);
    if (type === 'D9') return calculateNavamsaPosition(longitude);
    if (type === 'D10') return calculateDashamsaPosition(longitude);
    if (type === 'D12') return calculateDwadashamsaPosition(longitude);
    if (type === 'D16') return calculateShodashamsaPosition(longitude);
    if (type === 'D20') return calculateVimsamsaPosition(longitude);
    if (type === 'D24') return calculateChaturvimsamsaPosition(longitude);
    if (type === 'D27') return calculateSaptavimsamsaPosition(longitude);
    if (type === 'D30') return calculateTrimsamsaPosition(longitude);
    if (type === 'D40') return calculateKhavedamsaPosition(longitude);
    if (type === 'D45') return calculateAkshavedamsaPosition(longitude);
    if (type === 'D60') return calculateShashtyamsaPosition(longitude);
    throw new Error(`No position calculator registered for divisional chart type: ${type}`);
  };

  // 1. Calculate Divisional Ascendant
  const ascendantLongitude = (birthChart as any).ascendant?.totalLongitude ?? birthChart.lagna?.longitude ?? 0;
  const ascendant = calcPosition(ascendantLongitude);

  // 2. Calculate Divisional Planetary Positions
  const planets = {} as Record<PlanetName, DivisionalPosition>;
...
export function calculateAllDivisionalCharts(
  ...
  const allCharts = {} as Record<DivisionalChartType, DivisionalChart>;
  const vargaKeys = Object.keys(DIVISIONAL_PROFILES) as DivisionalChartType[];
 ...
 */
export function analyzeDivisionalChart(
  divChart: DivisionalChart,
  d1Analysis?: ChartAnalysisResult
): DivisionalChartAnalysis {
  const planetAnalyses: DivisionalPlanetAnalysis[] = [];
 ...
 */
export function compareRashiAndNavamsa(
  birthChart: BirthChart,
  d9Chart: DivisionalChart,
  d1Analysis?: ChartAnalysisResult
): VargaComparisonResult {
  const items: VargaComparisonItem[] = [];
 ...
 */
export function compareRashiAndDashamsa(
  birthChart: BirthChart,
  d10Chart: DivisionalChart,
  d1Analysis?: ChartAnalysisResult
): VargaDashamsaComparisonResult {
  const items: VargaDashamsaComparisonItem[] = [];
  const sameSignPlanets: PlanetName[] = [];
  ...
  for (const name of PLANET_NAMES) {
    ...
    if (!d1PlanetPos || !d10Pos) continue;
