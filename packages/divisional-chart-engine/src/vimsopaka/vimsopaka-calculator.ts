...
import { calculatePlanetDignity, PlanetFact, PrimaryDignity } from '@vedica/analysis-engine';
import {
  DivisionalChart,
  DivisionalChartType,
  PlanetVimsopakaBala,
...
export const SHODASHAVARGA_WEIGHTS: Record<DivisionalChartType, number> = {
  D1: 3.5,
  D2: 1.0,
  D3: 1.0,
  D4: 0.5,
...
export const SHADVARGA_WEIGHTS: Partial<Record<DivisionalChartType, number>> = {
  D1: 6.0,
  D2: 2.0,
  D3: 4.0,
  D9: 5.0,
...
export const SAPTAVARGA_WEIGHTS: Partial<Record<DivisionalChartType, number>> = {
  D1: 5.0,
  D2: 2.0,
  D3: 3.0,
  D7: 2.5,
...
export const DASAVARGA_WEIGHTS: Partial<Record<DivisionalChartType, number>> = {
  D1: 3.0,
  D2: 1.5,
  D3: 1.5,
  D7: 1.5,
...
const DIGNITY_RATIOS: Record<PrimaryDignity, number> = {
  EXALTED: 1.0, // 20/20
...
export function calculateVimsopakaBala(
  ...
  scheme: VimsopakaScheme = 'SHODASHAVARGA'
): VimsopakaBalaReport {
  let weightsMap: Partial<Record<DivisionalChartType, number>> = SHODASHAVARGA_WEIGHTS;
  if (scheme === 'SHADVARGA') weightsMap = SHADVARGA_WEIGHTS;
  if (scheme === 'SAPTAVARGA') weightsMap = SAPTAVARGA_WEIGHTS;
  if (scheme === 'DASAVARGA') weightsMap = DASAVARGA_WEIGHTS;
  ...
  const planetsReport = {} as Record<PlanetName, PlanetVimsopakaBala>;
  const ranking: { planet: PlanetName; score: number }[] = [];

  for (const planet of classicalPlanets) {
    const vargaScores: VimsopakaVargaScore[] = [];
    let totalScore = 0;

    for (const [chartTypeKey, weight] of Object.entries(weightsMap)) {
      const chartType = chartTypeKey as DivisionalChartType;
      if (!weight) continue;

      let rashiSign: any = null;
      ...
      if (chartType === 'D1') {
        ...
        if (d1Pos) {
          rashiSign = (d1Pos as any).rashi ?? (d1Pos as any).sign;
          longitudeInSign = (d1Pos as any).degreesInRashi ?? ((d1Pos as any).longitude % 30);
          absLongitude = (d1Pos as any).longitude ?? 0;
        }
      } else {
    ...
    let grade: 'EXCELLENT' | 'GOOD' | 'MODERATE' | 'WEAK' = 'MODERATE';
    if (roundedScore >= 15) grade = 'EXCELLENT';
    else if (roundedScore >= 12) grade = 'GOOD';
    else if (roundedScore >= 8) grade = 'MODERATE';
    else grade = 'WEAK';
