...
export function getElementCategory(rashiId) {
    const normId = ((rashiId - 1) % 12) + 1;
    if ([1, 5, 9].includes(normId))
        return 'FIRE';
    if ([2, 6, 10].includes(normId))
        return 'EARTH';
    if ([3, 7, 11].includes(normId))
        return 'AIR';
    return 'WATER';
}
/**
 ...
 */
export function calculateSaptavimsamsaPosition(sourceLongitude) {
    ...
    if (element === 'EARTH')
        startSignId = 4; // Cancer
    if (element === 'AIR')
        startSignId = 7; // Libra
    if (element === 'WATER')
        startSignId = 10; // Capricorn
