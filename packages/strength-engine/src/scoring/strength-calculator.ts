...
import {
  CLASSICAL_PLANETS,
  calculateRelationshipMatrix,
} from '../relationships/relationship-engine.js';
...
import {
  evaluateCombustionFactor,
  evaluateRetrogradeFactor,
} from '../conditions/combustion-retrograde.js';
import { evaluateD1D9Comparison } from '../comparison/d1-d9-comparison.js';

export function calculateSinglePlanetStrength(
  planet: PlanetName,
  chart: BirthChart,
  analysis: ChartAnalysisResult,
  relationships: RelationshipMatrix,
  d9Chart?: DivisionalChart,
  profile: StrengthProfile = PERSONAL_STRENGTH_V1
): PlanetStrengthAnalysis {
  ...
  // 2. Dignity Factor
  const dignityResult = evaluateDignityStrength(
    planet,
    analysis,
    signLordRel,
    profile
  ...
  const conjunctions = analysis.conjunctions.filter(
    (c) => (c.planetA === planet || c.planetB === planet) && c.detected
  );
  ...
  let overallStrength: OverallStrength = 'MODERATE';
...
export function evaluateStrengthEngine(
  chart: BirthChart,
  analysis: ChartAnalysisResult,
  d9Chart?: DivisionalChart,
  profile: StrengthProfile = PERSONAL_STRENGTH_V1
  ...
  const planets: PlanetStrengthAnalysis[] = CLASSICAL_PLANETS.map((p) =>
    calculateSinglePlanetStrength(p, chart, analysis, relationships, d9Chart, profile)
  );
...
function getSignLordName(signName: string): PlanetName {
