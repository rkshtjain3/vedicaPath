...
export async function generateMonthlyForecast(natalChart, options) {
    ...
    for (let m = 1; m <= 12; m++) {
        ...
        const dashaClimate = extractDashaClimateForDate(midMonthDate, options?.dashaData, natalChart);
        // 4. Domain Pulse Calculation (Career, Wealth, Relationships, Health, Focus)
        const domainPulse = calculateMonthlyDomainPulse(sunTransit.houseFromLagna, sunSav, transitsList, dashaClimate);
        // 5. Cosmic Weather & Atmosphere
        const cosmicWeather = evaluateCosmicWeather(domainPulse, dashaClimate, sunSav);
        // 6. Key Milestones / Dates
        const keyDates = generateMonthlyKeyDates(currentYear, m, sankrantiInfo, sunTransit.houseFromLagna, transitsList);
        ...
        const astrologicalWhy = [
            {
                ...
                evidenceStrength: sunSav >= 28 ? 'HIGH' : 'MEDIUM',
            },
            {
    ...
    const sortedByScore = [...monthsData].sort((a, b) => b.cosmicWeather.score - a.cosmicWeather.score);
...
function extractDashaClimateForDate(date, dashaData, natalChart) {
    let maha = 'Jupiter';
    let antar = 'Saturn';
...
function calculateMonthlyDomainPulse(sunHouse, sunSav, transits, dashaClimate) {
    ...
    return {
        career: {
            score: careerScore,
            trend: careerScore > 80 ? 'PEAK' : careerScore > 65 ? 'RISING' : 'STABLE',
        ...
        wealth: {
            score: wealthScore,
            trend: wealthScore > 80 ? 'PEAK' : wealthScore > 65 ? 'RISING' : 'CONSOLIDATING',
...
function generateMonthlyKeyDates(year, month, sankranti, sunHouse, transits) {
    const dates = [];
...
function getSattvicMonthlyFocus(signId, mahaLord) {
...
//# sourceMappingURL=monthly-evaluator.js.map