...
export function mapLordContextToTimingEvidence(periodId, lordContext, engineData) {
    const evidenceList = [];
    ...
    if (natal.house && natal.sign) {
        ...
        const primaryDomain = domains[0] || 'CAREER';
        evidenceList.push(createTimingEvidence({
    ...
    for (const y of yogas) {
        const primaryDomain = y.category?.toLowerCase().includes('dhana') ? 'WEALTH' : (y.category?.toLowerCase().includes('raja') ? 'CAREER' : (domains[0] || 'CAREER'));
        evidenceList.push(createTimingEvidence({
...
//# sourceMappingURL=dasha-evidence-mapper.js.map