...
import { ChartAnalysisResultInput, YogaAnalysisResult, YogaProfile, YogaResult } from './types/yoga-types.js';
...
export function evaluateYogaEngine(
  chart: BirthChart,
  analysis: ChartAnalysisResultInput,
  profile: YogaProfile = PERSONAL_YOGA_V1
): YogaAnalysisResult {
  const { planetFacts, houseLordFacts, conjunctions, aspects, dignities } = analysis;

  const allResults: YogaResult[] = [
