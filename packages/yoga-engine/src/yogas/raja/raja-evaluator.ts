import { isKendraHouse, isTrikonaHouse, isTrikHouse } from '../../helpers/house-helpers.js';
...
export function evaluateRajaYogas(
  ...
  const dkConditions: YogaCondition[] = [];
  const dkEvidence: YogaEvidence[] = [];
  ...
  const ktConditions: YogaCondition[] = [];
  const ktEvidence: YogaEvidence[] = [];
  ...
  for (const cfg of viparitaConfigs) {
    const vConditions: YogaCondition[] = [];
    const vEvidence: YogaEvidence[] = [];
