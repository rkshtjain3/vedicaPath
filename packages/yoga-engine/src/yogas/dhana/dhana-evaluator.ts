...
import { YogaResult, YogaCondition, YogaEvidence, HouseLordFactInput, PlanetFactInput, ConjunctionResultInput, VedicAspectInput } from '../../types/yoga-types.js';
...
export function evaluateDhanaYogas(
  ...
  const checkLordPairDhana = (
    ...
    const conditions: YogaCondition[] = [];
    ...
    if (validLords && lordA && lordB) {
      const rel = evaluatePlanetRelationship(
        lordA,
        lordB,
        houseA,
        houseB,
  ...
  results.push(
    checkLordPairDhana(
      ...
      2,
      11,
      '2nd Lord (Wealth) and 11th Lord (Gains) are associated via conjunction, aspect, or sign exchange'
    )
  );
  ...
  results.push(
    checkLordPairDhana(
      'DHANA_YOGA_2_5',
      'Dhana Yoga (2nd & 5th Lords)',
      2,
      5,
      '2nd Lord (Wealth) and 5th Lord (Speculation/Purva Punya) are associated'
    )
  );
  ...
  results.push(
    checkLordPairDhana(
      'DHANA_YOGA_5_9',
      'Dhana Yoga (5th & 9th Lords)',
      5,
      9,
      '5th Lord and 9th Lord (Lakshmi Sthanas / Trines) are associated'
    )
  );
  ...
  const d9Conditions: YogaCondition[] = [];
  const d9Evidence: YogaEvidence[] = [];
  ...
  const d11Conditions: YogaCondition[] = [];
  const d11Evidence: YogaEvidence[] = [];
