...
import { YogaResult, YogaCondition, YogaEvidence, PlanetFactInput, PlanetDignityInput } from '../../types/yoga-types.js';
...
export function evaluateLunarYogas(
  ...
  // 1. GAJAKESARI YOGA
  const gajakesariConditions: YogaCondition[] = [];
  const gajakesariEvidence: YogaEvidence[] = [];
  ...
  let secondFromMoonPlanets: string[] = [];
  let twelfthFromMoonPlanets: string[] = [];
  let kendraFromLagnaPlanets: string[] = [];
  let kendraFromMoonPlanets: string[] = [];
  ...
  if (moonFact) {
    ...
    for (const p of planetFacts) {
      if (!eligiblePlanets.includes(p.planet)) continue;
