...
export function evaluateDomainTransitEvidence(transits, aspects, conjunctions, houseContexts) {
    const domains = [
        'CAREER',
        'WEALTH',
        'RELATIONSHIPS',
            ...
            if (relevantHouses.includes(hc.houseFromLagna)) {
                const direction = hc.classification === 'SUPPORTIVE_CONTEXT'
                    ...
                    : hc.classification === 'CHALLENGING_CONTEXT'
                        ? 'CHALLENGING'
                        : 'NEUTRAL';
                domEvidenceList.push(createTransitEvidence(`TR-HOUSE-${hc.planet}-${dom}`, hc.planet, dom, direction, 1.0, `RULE-TR-HOUSE-${hc.houseFromLagna}`, `Transiting ${hc.planet} in natal house ${hc.houseFromLagna}`, hc.whyEvidence));
            }
        }
        // Aspects
        for (const asp of aspects) {
            if (asp.natalTarget === 'Lagna' && relevantHouses.includes(asp.aspectHouseDistance)) {
                domEvidenceList.push(createTransitEvidence(`TR-ASP-${asp.transitingPlanet}-${dom}`, asp.transitingPlanet, dom, 'SUPPORTIVE', 0.8, `RULE-TR-ASP-${asp.aspectType}`, `Transiting ${asp.transitingPlanet} aspects natal house ${asp.aspectHouseDistance}`, asp.whyEvidence));
            }
        }
        // Conjunctions
        for (const conj of conjunctions) {
            domEvidenceList.push(createTransitEvidence(`TR-CONJ-${conj.transitingPlanet}-${conj.natalPlanet}-${dom}`, conj.transitingPlanet, dom, 'SUPPORTIVE', 1.2, `RULE-TR-CONJ-${conj.transitingPlanet}-${conj.natalPlanet}`, `Transiting ${conj.transitingPlanet} conjunct natal ${conj.natalPlanet}`, conj.whyEvidence));
        }
