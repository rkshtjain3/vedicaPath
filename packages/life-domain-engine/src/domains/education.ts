...
export function evaluateEducationDomain(engineData: any): DomainEvaluationResult {
  ...
  const h5LordFact = analysis?.houseLordFacts?.find((hl: any) => hl.house === 5);
  const h5Fact = analysis?.houseFacts?.find((h: any) => h.house === 5);
  ...
  if (yogaAnalysis?.yogas) {
    const eduYogas = yogaAnalysis.yogas.filter((y: any) =>
      ['Budhaditya Yoga', 'Saraswati Yoga', 'Gaja Kesari Yoga'].includes(y.name)
    );
  ...
  return {
    ...
    relevantPlanetaryIndicators: [
      {
        ...
        sign: astrology?.planets?.find((p: any) => p.planet === 'Mercury')?.sign?.name || 'N/A',
        house: astrology?.planets?.find((p: any) => p.planet === 'Mercury')?.house || 1,
        dignity: astrology?.planets?.find((p: any) => p.planet === 'Mercury')?.dignity || 'NEUTRAL',
        strengthScore: strengthAnalysis?.planets?.find((p: any) => p.planet === 'Mercury')?.score || 0,
    ...
    relevantHouses: [
      {
        house: 4,
        sign: analysis?.houseFacts?.find((h: any) => h.house === 4)?.sign?.name || 'N/A',
        lord: h4LordFact?.lord || 'N/A',
        lordDignity: h4LordFact?.dignity || 'N/A',
        occupants: analysis?.houseFacts?.find((h: any) => h.house === 4)?.planets || [],
      },
      {
        house: 5,
        sign: analysis?.houseFacts?.find((h: any) => h.house === 5)?.sign?.name || 'N/A',
        lord: h5LordFact?.lord || 'N/A',
        lordDignity: h5LordFact?.dignity || 'N/A',
        occupants: analysis?.houseFacts?.find((h: any) => h.house === 5)?.planets || [],
      },
    ],
    relevantYogas: [],
    divisionalEvidence: [],
