...
export function evaluateCareerDomain(engineData: any): DomainEvaluationResult {
  ...
  const relevantPlanetaryIndicators = [
    ...
    },
    {
      ...
      sign: astrology?.planets?.find((p: any) => p.planet === 'Saturn')?.sign?.name || 'N/A',
      house: astrology?.planets?.find((p: any) => p.planet === 'Saturn')?.house || 1,
      dignity: astrology?.planets?.find((p: any) => p.planet === 'Saturn')?.dignity || 'NEUTRAL',
      strengthScore: strengthAnalysis?.planets?.find((p: any) => p.planet === 'Saturn')?.score || 0,
