...
export function evaluateWealthDomain(engineData: any): DomainEvaluationResult {
  ...
  const h2LordFact = analysis?.houseLordFacts?.find((hl: any) => hl.house === 2);
  const h11LordFact = analysis?.houseLordFacts?.find((hl: any) => hl.house === 11);
  const h2Fact = analysis?.houseFacts?.find((h: any) => h.house === 2);
  const h11Fact = analysis?.houseFacts?.find((h: any) => h.house === 11);
  ...
  const h12LordFact = analysis?.houseLordFacts?.find((hl: any) => hl.house === 12);
  ...
  if (h2Fact?.planets?.length) {
    ...
    const hasBenefic = h2Fact.planets.some((p: string) => benefics.includes(p));
    const item = createEvidenceItem({
  ...
  if (yogaAnalysis?.yogas) {
    const dhanaYogas = yogaAnalysis.yogas.filter((y: any) =>
      ['Dhana Yoga', 'Laxmi Yoga', 'Vasumati Yoga', 'Chandra Mangala Yoga'].includes(y.name)
    );
  ...
  if (currentMD) {
    ...
    const isConnWealth = [2, 11, 5, 9].includes(
      analysis?.houseLordFacts?.find((hl: any) => hl.lord === mdLord)?.house
    );
  ...
  return {
    ...
    relevantPlanetaryIndicators: [
      {
        ...
        sign: astrology?.planets?.find((p: any) => p.planet === 'Jupiter')?.sign?.name || 'N/A',
        house: astrology?.planets?.find((p: any) => p.planet === 'Jupiter')?.house || 1,
        dignity: astrology?.planets?.find((p: any) => p.planet === 'Jupiter')?.dignity || 'NEUTRAL',
        strengthScore: strengthAnalysis?.planets?.find((p: any) => p.planet === 'Jupiter')?.score || 0,
        shadbalaRatio: jupiterInfo.ratio || 1.0,
      },
      {
        planet: 'Venus',
        role: 'Asset & Prosperity Significator',
        sign: astrology?.planets?.find((p: any) => p.planet === 'Venus')?.sign?.name || 'N/A',
        house: astrology?.planets?.find((p: any) => p.planet === 'Venus')?.house || 1,
        dignity: astrology?.planets?.find((p: any) => p.planet === 'Venus')?.dignity || 'NEUTRAL',
        strengthScore: strengthAnalysis?.planets?.find((p: any) => p.planet === 'Venus')?.score || 0,
    ...
    relevantHouses: [
      {
        house: 2,
        sign: analysis?.houseFacts?.find((h: any) => h.house === 2)?.sign?.name || 'N/A',
        lord: h2LordFact?.lord || 'N/A',
        lordDignity: h2LordFact?.dignity || 'N/A',
        occupants: analysis?.houseFacts?.find((h: any) => h.house === 2)?.planets || [],
      },
      {
        house: 11,
        sign: analysis?.houseFacts?.find((h: any) => h.house === 11)?.sign?.name || 'N/A',
        lord: h11LordFact?.lord || 'N/A',
        lordDignity: h11LordFact?.dignity || 'N/A',
        occupants: analysis?.houseFacts?.find((h: any) => h.house === 11)?.planets || [],
      },
    ],
