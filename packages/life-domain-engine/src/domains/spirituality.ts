...
export function evaluateSpiritualityDomain(engineData: any): DomainEvaluationResult {
  ...
  const h9LordFact = analysis?.houseLordFacts?.find((hl: any) => hl.house === 9);
  const h12LordFact = analysis?.houseLordFacts?.find((hl: any) => hl.house === 12);
  const h9Fact = analysis?.houseFacts?.find((h: any) => h.house === 9);
  const h12Fact = analysis?.houseFacts?.find((h: any) => h.house === 12);
  ...
  const ketuFact = astrology?.planets?.find((p: any) => p.planet === 'Ketu');
  ...
  if (yogaAnalysis?.yogas) {
    const spiritYogas = yogaAnalysis.yogas.filter((y: any) =>
      ['Sanyasa Yoga', 'Gaja Kesari Yoga', 'Hamsa Yoga', 'Kedar Yoga'].includes(y.name)
    );
  ...
  return {
    ...
    relevantHouses: [
      {
        house: 9,
        sign: analysis?.houseFacts?.find((h: any) => h.house === 9)?.sign?.name || 'N/A',
        lord: h9LordFact?.lord || 'N/A',
        lordDignity: h9LordFact?.dignity || 'N/A',
        occupants: analysis?.houseFacts?.find((h: any) => h.house === 9)?.planets || [],
      },
      {
        house: 12,
        sign: analysis?.houseFacts?.find((h: any) => h.house === 12)?.sign?.name || 'N/A',
        lord: h12LordFact?.lord || 'N/A',
        lordDignity: h12LordFact?.dignity || 'N/A',
        occupants: analysis?.houseFacts?.find((h: any) => h.house === 12)?.planets || [],
      },
    ],
    relevantYogas: [],
    divisionalEvidence: [],
