    ...
    relevantPlanetaryIndicators: [
      {
        ...
        sign: astrology?.planets?.find((p: any) => p.planet === 'Mars')?.sign?.name || 'N/A',
        house: astrology?.planets?.find((p: any) => p.planet === 'Mars')?.house || 1,
        dignity: astrology?.planets?.find((p: any) => p.planet === 'Mars')?.dignity || 'NEUTRAL',
        strengthScore: strengthAnalysis?.planets?.find((p: any) => p.planet === 'Mars')?.score || 0,
