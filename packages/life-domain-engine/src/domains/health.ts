...
export function evaluateHealthDomain(engineData: any): DomainEvaluationResult {
  ...
  const HEALTH_DISCLAIMER =
    'This section is not medical advice and does not diagnose or predict medical conditions. It presents traditional symbolic astrological indicators for self-reflection only.';
  ...
  const h1LordFact = analysis?.houseLordFacts?.find((hl: any) => hl.house === 1);
  const h1Fact = analysis?.houseFacts?.find((h: any) => h.house === 1);
  ...
  if (h1Fact && Array.isArray(h1Fact.planets) && h1Fact.planets.length > 0) {
    ...
    const h1Benefics = h1Fact.planets.filter((p: string) => benefics.includes(p));
  ...
  const h6LordFact = analysis?.houseLordFacts?.find((hl: any) => hl.house === 6);
  const h6Fact = analysis?.houseFacts?.find((h: any) => h.house === 6);
  const h8Fact = analysis?.houseFacts?.find((h: any) => h.house === 8);
  ...
  const h6Malefics = h6Fact ? h6Fact.planets.filter((p: any) => malefics.includes(p)) : [];
  const h8Malefics = h8Fact ? h8Fact.planets.filter((p: any) => malefics.includes(p)) : [];
  ...
  if (currentMD) {
    ...
    const isTrikLord = [6, 8, 12].includes(
    ...
    relevantPlanetaryIndicators: [
      {
        ...
        sign: astrology?.planets?.find((p: any) => p.planet === 'Sun')?.sign?.name || 'N/A',
        house: astrology?.planets?.find((p: any) => p.planet === 'Sun')?.house || 1,
        dignity: astrology?.planets?.find((p: any) => p.planet === 'Sun')?.dignity || 'NEUTRAL',
        strengthScore: strengthAnalysis?.planets?.find((p: any) => p.planet === 'Sun')?.score || 0,
        shadbalaRatio: sunInfo.ratio || 1.0,
      },
      {
        planet: 'Moon',
        role: 'Mind & Psychological Resilience Karaka',
        sign: astrology?.planets?.find((p: any) => p.planet === 'Moon')?.sign?.name || 'N/A',
        house: astrology?.planets?.find((p: any) => p.planet === 'Moon')?.house || 1,
        dignity: astrology?.planets?.find((p: any) => p.planet === 'Moon')?.dignity || 'NEUTRAL',
        strengthScore: strengthAnalysis?.planets?.find((p: any) => p.planet === 'Moon')?.score || 0,
