import { DomainEvidenceItem, DomainScoringResult, DomainState } from '../types.js';

export function calculateDomainScoring(items: DomainEvidenceItem[]): DomainScoringResult {
  let supportScore = 0;
  ...
  for (const item of items) {
    const strengthMultiplier =
      item.strength === 'HIGH' ? 1.5 : item.strength === 'MEDIUM' ? 1.0 : 0.5;
  ...
  let state: DomainState = 'INSUFFICIENT_EVIDENCE';
