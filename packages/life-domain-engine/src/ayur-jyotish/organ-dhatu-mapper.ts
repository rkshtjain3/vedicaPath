...
const KAALA_PURUSHA_ZONES: Record<number, string> = {
...
const PLANET_DHATUS: Record<string, DhatuStatus['dhatuName']> = {
  Moon: 'Rasa (Plasma)',
  Sun: 'Asthi (Bone)',
  Mars: 'Rakta (Blood)',
...
export function diagnoseOrganDhatuAfflictions(chart: BirthChart): AyurJyotishOrganDhatuAnalysis {
  ...
  const zoneAfflictions: BodyZoneAffliction[] = [];
  ...
  // Dhatu evaluations
  const dhatuStatuses: DhatuStatus[] = [];
