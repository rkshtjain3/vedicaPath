...
const SIGN_ELEMENTS: Record<string, { vata: number; pitta: number; kapha: number }> = {
...
export function calculateAyurvedicDoshaProfile(calculationData: any): DoshaProfile {
  ...
  const sunPlanet = ast.planets?.find?.((p: any) => p.planet === 'Sun' || p.name === 'Sun');
  ...
  let primaryDosha: AyurvedicDosha = 'VATA';
  if (vataPct >= 40 && pittaPct >= 35) primaryDosha = 'VATA_PITTA';
  else if (pittaPct >= 40 && kaphaPct >= 35) primaryDosha = 'PITTA_KAPHA';
  else if (vataPct >= 40 && kaphaPct >= 35) primaryDosha = 'VATA_KAPHA';
  else if (vataPct >= pittaPct && vataPct >= kaphaPct) primaryDosha = 'VATA';
  else if (pittaPct >= vataPct && pittaPct >= kaphaPct) primaryDosha = 'PITTA';
  else primaryDosha = 'KAPHA';

  // Digestive Fire
  let digestiveFire: DoshaProfile['digestiveFireType'] = 'Vishama (Irregular/Vata)';
  ...
  let adaptogens: string[] = ['Ashwagandha (Grounding & Vata regulation)', 'Brahmi (Cognitive calming)', 'Tulsi (Immune support)'];
  let favoredFoods: string[] = ['Warm soups, cooked root vegetables, ghee, cumin & ginger', 'Moong dal khichdi', 'Warm herbal teas (CCF: Cumin-Coriander-Fennel)'];
  let avoidFoods: string[] = ['Ice-cold water, dry crackers, raw salads at night', 'Carbonated sodas', 'Excessive unfermented dairy & late-night heavy snacking'];
