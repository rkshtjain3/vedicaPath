    ...
    return {
        ...
        relevantPlanetaryIndicators: [
            ...
            },
            {
                ...
                sign: astrology?.planets?.find((p) => p.planet === 'Moon')?.sign?.name || 'N/A',
                house: astrology?.planets?.find((p) => p.planet === 'Moon')?.house || 1,
                dignity: astrology?.planets?.find((p) => p.planet === 'Moon')?.dignity || 'NEUTRAL',
                strengthScore: strengthAnalysis?.planets?.find((p) => p.planet === 'Moon')?.score || 0,
