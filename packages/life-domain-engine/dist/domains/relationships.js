...
export function evaluateRelationshipsDomain(engineData) {
    const evidenceItems = [];
    const whyEvidence = [];
    const { astrology, analysis, divisionalCharts, strengthAnalysis, shadbala, yogaAnalysis, dasha, timing, } = engineData;
    // 1. 7th House & 7th Lord
    const h7LordFact = analysis?.houseLordFacts?.find((hl) => hl.house === 7);
    const h7Fact = analysis?.houseFacts?.find((h) => h.house === 7);
    if (h7LordFact) {
