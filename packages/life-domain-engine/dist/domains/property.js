...
export function evaluatePropertyDomain(engineData) {
    const evidenceItems = [];
    const whyEvidence = [];
    ...
    const h4Fact = analysis?.houseFacts?.find((h) => h.house === 4);
    if (h4LordFact) {
    ...
    if (h4Fact?.planets?.length) {
        ...
        const hasBenefic = h4Fact.planets.some((p) => benefics.includes(p));
        const item = createEvidenceItem({
    ...
    if (yogaAnalysis?.yogas) {
        const propYogas = yogaAnalysis.yogas.filter((y) => ['Ruchaka Yoga'].includes(y.name));
        ...
        relevantPlanetaryIndicators: [
            {
                ...
                sign: astrology?.planets?.find((p) => p.planet === 'Mars')?.sign?.name || 'N/A',
                house: astrology?.planets?.find((p) => p.planet === 'Mars')?.house || 1,
                dignity: astrology?.planets?.find((p) => p.planet === 'Mars')?.dignity || 'NEUTRAL',
                strengthScore: strengthAnalysis?.planets?.find((p) => p.planet === 'Mars')?.score || 0,
