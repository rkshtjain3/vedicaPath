...
export function evaluateCareerDomain(engineData) {
    const evidenceItems = [];
    const whyEvidence = [];
    const { astrology, analysis, divisionalCharts, strengthAnalysis, shadbala, ashtakavarga, yogaAnalysis, rules, dasha, timing, transitAshtakavarga, crossChartAnalysis, } = engineData;
    ...
    const h10LordFact = analysis?.houseLordFacts?.find((hl) => hl.house === 10);
    const h10Fact = analysis?.houseFacts?.find((h) => h.house === 10);
    if (h10LordFact) {
    ...
    const d10AscSign = divisionalCharts?.d10Analysis?.ascendantSign ||
        divisionalCharts?.d10Chart?.ascendant?.sign?.name ||
        divisionalCharts?.d10Chart?.ascendant?.sign;
    const d10AscLord = divisionalCharts?.d10Analysis?.ascendantLord ||
    ...
    if (yogaAnalysis?.yogas) {
        const rajaYogas = yogaAnalysis.yogas.filter((y) => ['Raja Yoga', 'Dharma Karma Adhipati Yoga', 'Amala Yoga', 'Gaja Kesari Yoga'].includes(y.name));
    ...
    if (rules?.careerD10) {
        ...
        const triggeredD10Rules = d10RulesList.filter((r) => r.triggered);
    ...
    if (currentMD) {
        ...
        const isConn10 = analysis?.houseFacts?.find((h) => h.house === 10)?.planets?.includes(mdLord);
    ...
    const saturnTransit = timing?.transits?.planets?.find((p) => p.planet === 'Saturn');
    ...
    const relevantPlanetaryIndicators = [
        {
            ...
            sign: astrology?.planets?.find((p) => p.planet === 'Sun')?.sign?.name || 'N/A',
            house: astrology?.planets?.find((p) => p.planet === 'Sun')?.house || 1,
            dignity: astrology?.planets?.find((p) => p.planet === 'Sun')?.dignity || 'NEUTRAL',
            strengthScore: strengthAnalysis?.planets?.find((p) => p.planet === 'Sun')?.score || 0,
            shadbalaRatio: sunInfo.ratio || 1.0,
        },
        {
            planet: 'Saturn',
            role: 'Profession & Service Significator',
            sign: astrology?.planets?.find((p) => p.planet === 'Saturn')?.sign?.name || 'N/A',
            house: astrology?.planets?.find((p) => p.planet === 'Saturn')?.house || 1,
            dignity: astrology?.planets?.find((p) => p.planet === 'Saturn')?.dignity || 'NEUTRAL',
            strengthScore: strengthAnalysis?.planets?.find((p) => p.planet === 'Saturn')?.score || 0,
    ...
    const relevantHouses = [
        ...
        {
            house: 6,
            sign: analysis?.houseFacts?.find((h) => h.house === 6)?.sign?.name || 'N/A',
            ...
            occupants: analysis?.houseFacts?.find((h) => h.house === 6)?.planets || [],
        },
    ];
    return {
        domain: 'CAREER',
