export function retrieveYogaEvidence(calculationData, targetPlanet) {
    const items = [];
    ...
    if (yogaAnalysis.detectedYogas && Array.isArray(yogaAnalysis.detectedYogas)) {
        for (const y of yogaAnalysis.detectedYogas) {
            if (targetPlanet &&
                !y.involvedPlanets?.some((p) => p.toLowerCase() === targetPlanet.toLowerCase())) {
                continue;
            }
            items.push({
