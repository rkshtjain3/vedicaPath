export function retrieveLifeDomainEvidence(calculationData, targetDomain, targetPlanet) {
    const items = [];
    ...
    for (const [domKey, domData] of Object.entries(domains)) {
        const domainUpper = domKey.toUpperCase();
        if (targetDomain && domainUpper !== targetDomain)
            continue;
        if (!domData)
            continue;
        // Supporting Factors
        if (domData.supportingFactors && Array.isArray(domData.supportingFactors)) {
            for (const item of domData.supportingFactors) {
                if (targetPlanet && !item.description?.toLowerCase().includes(targetPlanet.toLowerCase()))
                    continue;
                items.push({
