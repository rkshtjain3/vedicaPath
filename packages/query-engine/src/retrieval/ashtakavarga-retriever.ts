...
export function retrieveAshtakavargaEvidence(calculationData: any, targetPlanet?: string): QueryEvidenceItem[] {
  ...
  if (av.bav && typeof av.bav === 'object') {
    for (const [pName, bData] of Object.entries(av.bav) as [string, any][]) {
      if (targetPlanet && pName.toLowerCase() !== targetPlanet.toLowerCase()) continue;
      if (bData && Array.isArray(bData.houseScores)) {
        const total = bData.totalPoints || bData.houseScores.reduce((acc: number, curr: any) => acc + (curr.score || 0), 0);
        items.push({
  ...
  if (transitAV.evaluations && Array.isArray(transitAV.evaluations)) {
    for (const tav of transitAV.evaluations) {
      if (targetPlanet && tav.planet?.toLowerCase() !== targetPlanet.toLowerCase()) continue;
