...
import {
  ...
  PalmistryInputPayload,
} from './types/palmistry-types.js';
import { assessImageQuality } from './quality/quality-assessor.js';
import {
  computeDigitalRatios,
  extractPalmarLandmarks,
} from './geometry/landmark-detector.js';
import { detectPalmarLines } from './lines/line-detector.js';
import { analyzePalmarMounts } from './mounts/mount-analyzer.js';
import {
  buildPalmistryObservations,
  evaluateHastRekhaRules,
} from './rules/hast-rekha-rules.js';

export function evaluatePalmistry(payload: PalmistryInputPayload = {}): PalmistryAnalysisResult {
  ...
  const pipelineStages = [
    { stageNumber: 1, stageName: 'Photo Input & Frame Ingestion', status: 'PASSED' as const, details: 'Palmar image ingested successfully.' },
    { stageNumber: 2, stageName: 'Image Quality Assessment', status: quality.passesQualityThreshold ? ('PASSED' as const) : ('WARNING' as const), details: `Sharpness: ${quality.sharpnessScore}/100, Visibility: ${quality.handVisibilityConfidence}%.` },
    { stageNumber: 3, stageName: 'Hand Detection & Segmentation', status: 'PASSED' as const, details: `Hand identified as ${handType} (${handDominance}).` },
    { stageNumber: 4, stageName: 'Landmark Detection', status: 'PASSED' as const, details: `Extracted 13 palmar landmarks; 2D:4D ratio = ${digitalRatios.ratio2D4D}.` },
    { stageNumber: 5, stageName: 'Line Detection', status: 'PASSED' as const, details: `Detected Life, Head, Heart, Fate, Sun, and Mercury lines.` },
    { stageNumber: 6, stageName: 'Mount/Finger Measurement', status: 'PASSED' as const, details: `Evaluated 7 Palmar Mounts; Shukra score = ${mounts.venus.score}, Guru score = ${mounts.jupiter.score}.` },
    { stageNumber: 7, stageName: 'Structured Observations', status: 'PASSED' as const, details: `Generated ${observations.length} normalized observation vectors.` },
    { stageNumber: 8, stageName: 'Deterministic Rules', status: 'PASSED' as const, details: `Triggered ${ruleResults.length} Hast Rekha AST rules.` },
    { stageNumber: 9, stageName: 'Structured Interpretation', status: 'PASSED' as const, details: 'Synthesized 4 core life domain insights.' },
  ];
