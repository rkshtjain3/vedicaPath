...
export function buildPlanetaryStrengthSection(strengthAnalysis, shadbala) {
    ...
    const strongPlanets = strengthAnalysis?.planets
        ?.filter((p) => p.overallStrength === 'STRONG' || p.overallStrength === 'VERY_STRONG')
        .map((p) => p.planet) || [];
    const challengedPlanets = strengthAnalysis?.planets
        ?.filter((p) => p.overallStrength === 'WEAK' || p.overallStrength === 'CHALLENGED')
        .map((p) => p.planet) || [];
    const summary = `Planetary Strength Engine evaluated ${strengthAnalysis?.planets?.length || 9} planets. Strongest indicators: ${strongPlanets.length > 0 ? strongPlanets.join(', ') : 'None'}. Challenged indicators: ${challengedPlanets.length > 0 ? challengedPlanets.join(', ') : 'None'}. Note: Current Shadbala implementation provides component-level evidence; components marked PARTIAL or FOUNDATION represent non-exhaustive classical scores.`;
    return {
