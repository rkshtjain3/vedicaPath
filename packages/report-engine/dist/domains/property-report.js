...
export function buildPropertyReportSection(rulesResult, timingResult, analysis) {
    ...
    if (rulesResult?.evaluatedRules) {
        for (const r of rulesResult.evaluatedRules) {
            if (r.active && (r.domain === 'PROPERTY' || r.domain === 'VEHICLES')) {
                rawEvidence.push(createEvidenceItem(`rule_${r.id}`, 'rules-engine', 'RULE_ACTIVATION', `${r.title}: ${r.description || r.evidence?.join(', ')}`, r.category === 'CHALLENGE' ? 'CHALLENGING' : 'SUPPORTIVE', r.weight >= 1.5 ? 'HIGH' : 'MEDIUM', 'PROPERTY', r.id, r));
            }
        }
    }
    const h4Lord = analysis?.houseLords?.find((hl) => hl.house === 4)?.lord || 'Mars/Moon';
    rawEvidence.push(createEvidenceItem('property_h4_lord', 'analysis-engine', 'HOUSE_4_LORD', `4th House Lord (Real Estate & Property) is ${h4Lord}.`, 'FACTUAL', 'MEDIUM', 'PROPERTY', `LORD_4_${h4Lord}`, { h4Lord }));
    if (timingResult?.domainTimelines) {
        const dt = timingResult.domainTimelines.find((t) => t.domain === 'PROPERTY' || t.domain === 'REAL_ESTATE');
        if (dt) {
            rawEvidence.push(createEvidenceItem('timing_property_timeline', 'timing-engine', 'TIMING_ACTIVITY', `Property Domain Timing status: ${dt.status} (Score: ${dt.score.toFixed(1)}).`, dt.status === 'SUPPORTIVE' || dt.status === 'HIGH_ACTIVITY'
                ? 'SUPPORTIVE'
                : dt.status === 'CHALLENGING'
                    ? 'CHALLENGING'
                    : 'NEUTRAL', 'HIGH', 'PROPERTY', 'TIMING_PROPERTY', dt));
        }
    }
    ...
    const summary = `Property & Real Estate Domain Report: Evaluated ${prioritized.length} evidence items from 4th House Lordship, Rules, and Timing engines. Convergence Level: ${convergence.level}. Mixed Signals: ${contradiction.mixedSignals ? 'YES' : 'NO'}. Framework Confidence: ${confidence}.`;
    return {
        id: 'property_report',
