import { ReportEvidence } from '../types/evidence-types.js';

export interface PartitionedEvidence {
  supportive: ReportEvidence[];
  challenging: ReportEvidence[];
  neutral: ReportEvidence[];
  factual: ReportEvidence[];
  mixedSignals: boolean;
}

export function partitionEvidenceByConflict(evidenceList: ReportEvidence[]): PartitionedEvidence {
  const supportive: ReportEvidence[] = [];
  const challenging: ReportEvidence[] = [];
  const neutral: ReportEvidence[] = [];
  const factual: ReportEvidence[] = [];

  for (const item of evidenceList) {
    if (item.classification === 'SUPPORTIVE') {
      supportive.push(item);
    } else if (item.classification === 'CHALLENGING') {
      challenging.push(item);
    } else if (item.classification === 'FACTUAL') {
      factual.push(item);
    } else {
      neutral.push(item);
    }
  }

  const mixedSignals = supportive.length > 0 && challenging.length > 0;

  return {
    supportive,
    challenging,
    neutral,
    factual,
    mixedSignals,
  };
}
