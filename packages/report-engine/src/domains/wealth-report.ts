...
export function buildWealthReportSection(
  rulesResult: any,
  timingResult: any,
  ashtakavarga: any,
  yogaAnalysis: any
): DomainReportSection {
  const domain: LifeDomain = 'WEALTH';
      ...
      if (y.status === 'DETECTED' && y.category === 'DHANA') {
        rawEvidence.push(
          createEvidenceItem(
            ...
            'yoga-engine',
            'DHANA_YOGA',
            `${y.name} (Dhana Yoga) detected, indicating wealth lord associations.`,
            'SUPPORTIVE',
            'HIGH',
            'WEALTH',
            y.id,
  ...
  if (ashtakavarga?.sav?.signPoints) {
    ...
    rawEvidence.push(
      createEvidenceItem(
        'ashtakavarga_wealth_sav',
        'ashtakavarga-engine',
        'SAV_POINTS',
        `SAV total bindus support general wealth and resource stability.`,
        'FACTUAL',
        'MEDIUM',
        'WEALTH',
        'SAV_WEALTH',
  ...
  if (timingResult?.domainTimelines) {
    const dt = timingResult.domainTimelines.find((t: any) => t.domain === 'WEALTH' || t.domain === 'FINANCE');
    if (dt) {
      rawEvidence.push(
        createEvidenceItem(
          'timing_wealth_timeline',
          'timing-engine',
          'TIMING_ACTIVITY',
          `Wealth Domain Timing status: ${dt.status} (Score: ${dt.score.toFixed(1)}).`,
  ...
  const summary = `Wealth & Assets Domain Report: Evaluated ${prioritized.length} evidence items from Rules, Dhana Yogas, Ashtakavarga, and Timing engines. Convergence Level: ${
    convergence.level
