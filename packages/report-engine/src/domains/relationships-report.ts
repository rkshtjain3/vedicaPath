...
export function buildRelationshipsReportSection(
  rulesResult: any,
  timingResult: any,
  analysis: any,
  d9Analysis: any
): DomainReportSection {
  const domain: LifeDomain = 'RELATIONSHIPS';
  ...
  const h7Lord = analysis?.houseLords?.find((hl: any) => hl.house === 7)?.lord || 'Venus/Jupiter';
  rawEvidence.push(
    createEvidenceItem(
      'relationship_h7_lord',
      'analysis-engine',
      'HOUSE_7_LORD',
      `7th House Lord is ${h7Lord}. D9 Navamsa chart provides subtle relation facts.`,
      'FACTUAL',
      'MEDIUM',
      'RELATIONSHIPS',
      `LORD_7_${h7Lord}`,
  ...
  if (timingResult?.domainTimelines) {
    const dt = timingResult.domainTimelines.find((t: any) => t.domain === 'RELATIONSHIPS' || t.domain === 'MARRIAGE');
    if (dt) {
      rawEvidence.push(
        createEvidenceItem(
          'timing_relationship_timeline',
          'timing-engine',
          'TIMING_ACTIVITY',
          `Relationships Domain Timing status: ${dt.status} (Score: ${dt.score.toFixed(1)}).`,
  ...
  const summary = `Relationships Domain Report: Evaluated ${prioritized.length} evidence items from D1 Rashi, D9 Navamsa, Rules, and Timing engines. Convergence Level: ${
    convergence.level
