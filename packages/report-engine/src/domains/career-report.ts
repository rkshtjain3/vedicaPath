...
export function buildCareerReportSection(
  rulesResult: any,
  timingResult: any,
      ...
      if (r.active) {
        rawEvidence.push(
          createEvidenceItem(
            ...
            'rules-engine',
            'D10_CAREER_RULE',
            `D10 Dashamsa Fact (${r.title}): ${r.description || r.evidence?.join(', ')}`,
            r.category === 'CHALLENGE' ? 'CHALLENGING' : 'SUPPORTIVE',
            'HIGH',
            'CAREER',
            r.id,
    ...
    for (const f of crossChartAnalysis.career.facts) {
      rawEvidence.push(
        createEvidenceItem(
          ...
          'CROSS_CHART_FACT',
          `Cross-chart fact: ${f.title} (${f.description})`,
          'FACTUAL',
          'MEDIUM',
          'CAREER',
          f.id,
      ...
      if (y.status === 'DETECTED' && (y.category === 'MAHAPURUSHA' || y.category === 'RAJA')) {
        rawEvidence.push(
          createEvidenceItem(
            ...
            'yoga-engine',
            'YOGA_CAREER_SUPPORT',
            `${y.name} (${y.category}) detected, providing structural support to chart authority.`,
            'SUPPORTIVE',
            'HIGH',
            'CAREER',
            y.id,
  ...
  if (timingResult?.domainTimelines) {
    const dt = timingResult.domainTimelines.find((t: any) => t.domain === 'CAREER');
    if (dt) {
      rawEvidence.push(
        createEvidenceItem(
          'timing_career_timeline',
          'timing-engine',
          'TIMING_ACTIVITY',
          `Career Domain Timing status: ${dt.status} (Weight: ${dt.score.toFixed(1)}).`,
  ...
  const summary = `Career Domain Report: Evaluated ${prioritized.length} evidence items across D1, D10 Dashamsa, Rules, Yogas, and Timing engines. Convergence Level: ${
    convergence.level
  }. Mixed Signals: ${contradiction.mixedSignals ? 'YES (both supportive and challenging factors exist)' : 'NO'}. Framework Confidence: ${confidence}.`;
