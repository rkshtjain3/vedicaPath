...
export function buildYogaSection(yogaAnalysis: any): ReportSection {
  const detectedEvidence = normalizeYogaEvidence(yogaAnalysis);

  const notDetectedYogas = (yogaAnalysis?.results || [])
    .filter((y: any) => y.status === 'NOT_DETECTED')
    .map((y: any) =>
      createEvidenceItem(
        `yoga_nd_${y.id}`,
        'yoga-engine',
        'YOGA_NOT_DETECTED',
        `${y.name} (${y.category}) condition check was not satisfied.`,
        'NEUTRAL',
        'LOW',
        'YOGAS',
        y.id,
  ...
  const detectedNames = (yogaAnalysis?.results || [])
    .filter((y: any) => y.status === 'DETECTED')
    .map((y: any) => y.name);

  const summary = `Classical Yoga Engine evaluated ${yogaAnalysis?.totalEvaluated || 24} canonical Yogas. Detected ${
    yogaAnalysis?.detectedCount || 0
  } Yogas: ${detectedNames.length > 0 ? detectedNames.join(', ') : 'None'}.`;
