...
export interface DigBalaDirectionPoint {
  planet: PlanetName;
  strongestHouse: number; // House where Dig Bala is maximum 60 Virupas
  strongestDirection: 'EAST' | 'SOUTH' | 'WEST' | 'NORTH';
  zeroHouse: number; // House 180° opposite strongest house where Dig Bala is 0 Virupas
  zeroLongitudeOffsetFromAscendant: number; // degrees from Lagna longitude
}
 ...
 */
export const DIG_BALA_POINTS: Record<PlanetName, DigBalaDirectionPoint> = {
  Mercury: {
    planet: 'Mercury',
    strongestHouse: 1,
...
export function calculateDigBala(
  ...
  methodology: DigBalaMethodology = 'EXACT_ANGULAR'
): ShadbalaComponent {
