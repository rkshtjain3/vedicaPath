...
import { DivisionalChart, calculateDivisionalChart } from '@vedica/divisional-chart-engine';
import {
  PlanetShadbala,
  ShadbalaEngineResult,
  ShadbalaProfile,
  RequiredStrengthFact,
  ShadbalaComponent,
} from './types/shadbala-types.js';
...
export const SEVEN_PLANETS: PlanetName[] = [
  'Sun',
  'Moon',
  'Mars',
  'Mercury',
...
export function evaluateShadbalaEngine(
  context: ShadbalaContext,
  profile: ShadbalaProfile = PERSONAL_SHADBALA_V1
): ShadbalaEngineResult {
  ...
  const planetShadbalaList: PlanetShadbala[] = [];
  const requiredStrengthMap: Record<PlanetName, RequiredStrengthFact> = {} as any;

  // Prepare 7-planet list for aspect calculations and temporal lords
  const sevenPlanetsArray: Array<{ planet: PlanetName; longitude: number }> = SEVEN_PLANETS.map((p) => {
  ...
  const sunPlanetPos = chart.planets.find((p) => p.planet === 'Sun') || {
    ...
    degreeInSign: 0,
    speed: 1,
    sign: { id: 1, name: 'Aries', ruler: 'Mars' } as any,
  };
  ...
  const moonPlanetPos = chart.planets.find((p) => p.planet === 'Moon') || {
    ...
    degreeInSign: 0,
    speed: 13,
    sign: { id: 4, name: 'Cancer', ruler: 'Moon' } as any,
  };
  ...
  for (const planet of SEVEN_PLANETS) {
    ...
    const sthanaBala = calculateSthanaBala(
      planet,
      planetPos.longitude,
      planetPos.degreeInSign,
      houseNumber,
      planetPos.sign,
      d9Sign,
      saptavargaja
    ...
    const kaalaBala = calculateKaalaBala({
      ...
      ayanamshaValue: (chart as any).siderealAyana ?? (chart as any).ayanamshaValue,
      allPlanets: sevenPlanetsArray,
    });

    // 7. Drik Bala
    const drikBala = calculateDrikBala(
      planet,
      planetPos.longitude,
      sevenPlanetsArray
    );
    ...
    const planetShadbala: PlanetShadbala = {
      planet,
      components: {
        sthana: sthanaBala,
        dig: digBala,
