...
import { PLANET_GENDER_CATEGORIES, PlanetGenderCategory } from './ojayugma-bala.js';
 ...
 */
export function getDrekkanaNumber(degreeInSign: number): DrekkanaNumber {
  const normDeg = degreeInSign % 30;
  if (normDeg < 10) return 1;
  if (normDeg < 20) return 2;
  return 3;
}
 ...
 */
export function calculateDrekkanaBala(planet: PlanetName, degreeInSign: number): SthanaBalaSubcomponent {
  const drekkana = getDrekkanaNumber(degreeInSign);
  const genderCategory: PlanetGenderCategory = PLANET_GENDER_CATEGORIES[planet] || 'NEUTRAL';

  let virupas = 0;
  if (genderCategory === 'MALE' && drekkana === 1) virupas = 15;
  else if (genderCategory === 'NEUTRAL' && drekkana === 2) virupas = 15;
  else if (genderCategory === 'FEMALE' && drekkana === 3) virupas = 15;
