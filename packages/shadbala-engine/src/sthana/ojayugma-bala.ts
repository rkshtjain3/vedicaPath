...
export const PLANET_GENDER_CATEGORIES: Record<PlanetName, PlanetGenderCategory> = {
  Sun: 'MALE',
  Mars: 'MALE',
  Jupiter: 'MALE',
  Moon: 'FEMALE',
 ...
 */
export function calculateOjayugmaBala(
  planet: PlanetName,
  d1Sign: RashiDetails,
  d9Sign?: RashiDetails
): SthanaBalaSubcomponent {
  const genderCategory = PLANET_GENDER_CATEGORIES[planet] || 'NEUTRAL';

  const isOddSign = (signId: number) => signId % 2 === 1;

  // D1 check
  const d1IsOdd = isOddSign(d1Sign.id);
  const d1GetsPoints =
    genderCategory === 'MALE' || genderCategory === 'NEUTRAL' ? d1IsOdd : !d1IsOdd;
  const d1Virupas = d1GetsPoints ? 15 : 0;

  // D9 check
  let d9Virupas = 0;
  let d9IsOdd: boolean | undefined;
  if (d9Sign) {
    d9IsOdd = isOddSign(d9Sign.id);
    const d9GetsPoints =
      genderCategory === 'MALE' || genderCategory === 'NEUTRAL' ? d9IsOdd : !d9IsOdd;
