import { PlanetName, PlanetPosition, RashiDetails } from '@vedica/astrology-core';
import { KaalaBalaComponent, KaalaSubcomponentName } from '../types/shadbala-types.js';
...
export interface KaalaBalaInput {
  planet: PlanetName;
  planetPos: { longitude: number; degreeInSign?: number; speed?: number; sign?: any };
  houseNumber: number;
  sunPlanetPos: { longitude: number; degreeInSign?: number; speed?: number; sign?: any };
  moonPlanetPos: { longitude: number; degreeInSign?: number; speed?: number; sign?: any };
  sunHouse: number;
 ...
 */
export function calculateKaalaBala(input: KaalaBalaInput): KaalaBalaComponent {
  const {
    planet,
    planetPos,
    sunPlanetPos,
  ...
  const subcomponents: Record<KaalaSubcomponentName, any> = {
