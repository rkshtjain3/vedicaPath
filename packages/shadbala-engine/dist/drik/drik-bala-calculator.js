 ...
 */
export function calculateDrikBala(planet, planetLongitude, allPlanets) {
    let totalVirupas = 0;
    const aspectDetails = [];
    const evidence = [`Planet: ${planet} (Drik Bala / Aspectual Strength Evaluation)`];
    for (const other of allPlanets) {
        ...
        if (drushtiVirupas > 0) {
            ...
            evidence.push(`Aspect from ${other.planet} (Separation: ${angle.toFixed(1)}°): Raw Drushti = ${drushtiVirupas.toFixed(1)} Virupas -> ${isBenefic ? 'Benefic' : 'Malefic'} net = ${netContribution > 0 ? '+' : ''}${netContribution.toFixed(2)} Virupas`);
        }
    }
 ...
 */
function calculateParashariDrushti(aspectingPlanet, angle) {
    let baseDrushti = 0;
    ...
    if (aspectingPlanet === 'Mars') {
        ...
        if (Math.abs(angle - 90) <= 15)
            baseDrushti = Math.max(baseDrushti, 60 - Math.abs(angle - 90) * 2);
        if (Math.abs(angle - 210) <= 15)
            baseDrushti = Math.max(baseDrushti, 60 - Math.abs(angle - 210) * 2);
    }
    else if (aspectingPlanet === 'Jupiter') {
        // Jupiter 5th aspect (120°) and 9th aspect (240°)
        if (Math.abs(angle - 120) <= 15)
            baseDrushti = Math.max(baseDrushti, 60 - Math.abs(angle - 120) * 2);
        if (Math.abs(angle - 240) <= 15)
            baseDrushti = Math.max(baseDrushti, 60 - Math.abs(angle - 240) * 2);
    }
    else if (aspectingPlanet === 'Saturn') {
        // Saturn 3rd aspect (60°) and 10th aspect (270°)
        if (Math.abs(angle - 60) <= 15)
            baseDrushti = Math.max(baseDrushti, 60 - Math.abs(angle - 60) * 2);
        if (Math.abs(angle - 270) <= 15)
            baseDrushti = Math.max(baseDrushti, 60 - Math.abs(angle - 270) * 2);
    }
