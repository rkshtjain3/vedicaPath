import { getPanchadhaMaitri, calculatePlanetDignity } from '@vedica/analysis-engine';
 ...
 */
export function calculateSaptavargajaBala(context) {
    let totalVirupas = 0;
    const evidence = [];
    const vargas = ['D1', 'D2', 'D3', 'D7', 'D9', 'D12', 'D30'];
    const missingVargas = [];
    const getPoints = (vargaName, sign, degreeInSign) => {
        // Determine Lord
        ...
        if (dignity.primaryDignity === 'MOOLATRIKONA') {
            ...
            if (!targetFact) {
                ...
                if (dignity.relationshipToSignLord === 'FRIENDLY') {
                    points = 15;
                    relStr = 'Mitra (Natural fallback)';
                }
                else if (dignity.relationshipToSignLord === 'ENEMY') {
                    points = 3.75;
                    relStr = 'Shatru (Natural fallback)';
                }
                else {
                    points = 7.5;
                    relStr = 'Sama (Natural fallback)';
                }
            }
            else {
                ...
                switch (panchadha) {
                    case 'ADHI_MITRA':
                        points = 22.5;
                        relStr = 'Adhi Mitra (Great Friend)';
                        break;
                    case 'MITRA':
                        points = 15;
                        relStr = 'Mitra (Friend)';
                    ...
                    case 'ADHI_SHATRU':
                        points = 1.875;
                        relStr = 'Adhi Shatru (Great Enemy)';
                        break;
                }
            }
        }
    ...
    // Evaluate D1
    getPoints('D1', context.d1Chart.planets.find((p) => p.planet === context.planet).sign, context.d1Fact.degreeInSign);
    // Evaluate other vargas
    const evaluateVarga = (name, chart) => {
        if (chart) {
