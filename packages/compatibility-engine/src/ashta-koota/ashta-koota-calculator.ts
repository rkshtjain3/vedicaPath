import { AshtaKootaResult, KootaScore } from '../types/compatibility-types.js';
...
export function calculateAshtaKoota(
  moonNakshatraA: number, // 1-27
  moonRashiA: number,     // 1-12
  moonNakshatraB: number,
  moonRashiB: number
): AshtaKootaResult {
  ...
  const getVarnaRank = (rashi: number) => {
    if ([4, 8, 12].includes(rashi)) return 4; // Brahmin
  ...
  const isAuspiciousTara = (c: number) => ![3, 5, 7].includes(c);
  let taraPts = 0;
  if (isAuspiciousTara(countAB) && isAuspiciousTara(countBA)) taraPts = 3;
  else if (isAuspiciousTara(countAB) || isAuspiciousTara(countBA)) taraPts = 1.5;
  scores.push({
    kootaName: 'Tara',
    maxPoints: 3,
  ...
  let ganaPts = 6;
  if (ganaA === ganaB) ganaPts = 6;
  else if ((ganaA === 'DEVA' && ganaB === 'MANUSHYA') || (ganaA === 'MANUSHYA' && ganaB === 'DEVA')) ganaPts = 5;
  else if ((ganaA === 'DEVA' && ganaB === 'RAKSHASA') || (ganaA === 'RAKSHASA' && ganaB === 'DEVA')) ganaPts = 1;
  else ganaPts = 0;
  scores.push({
    kootaName: 'Gana',
    maxPoints: 6,
  ...
  let grade: AshtaKootaResult['compatibilityGrade'] = 'AVERAGE';
  if (totalObtained >= 28) grade = 'EXCELLENT';
  else if (totalObtained >= 21) grade = 'GOOD';
  else if (totalObtained >= 18) grade = 'AVERAGE';
  else if (totalObtained >= 14) grade = 'BELOW_AVERAGE';
  else grade = 'POOR';
