...
import { CompatibilityAnalysisResult } from './types/compatibility-types.js';
...
function getNakshatraIndexFromLongitude(longitude: number): number {
...
export function evaluateCompatibility(
  ...
  const moonA = chartA.planets.find((p: PlanetPosition) => p.planet === 'Moon');
  const moonB = chartB.planets.find((p: PlanetPosition) => p.planet === 'Moon');
