...
export function detectNaturalLanguage(text: string): 'en' | 'hi' | 'hinglish' {
  if (/[\u0900-\u097F]/.test(text)) {
    return 'hi';
  }
...
export interface AstrologicalRAGContext {
  ...
  followUpSuggestions: Array<{ en: string; hi: string; query: string }>;
  planetsDetail: Array<{
    planet: string;
    sign: string;
    house: number;
  ...
  topShadbalaPlanets: Array<{ planet: string; rupas: number; percentage: number }>;
  verifiedYogas: Array<{ name: string; category?: string; description?: string }>;
 ...
 */
export function classifyLifeDomain(question: string): { topic: LifeDomainTopic; confidence: 'HIGH' | 'MEDIUM' | 'GENERAL' } {
  const q = question.toLowerCase().trim();

  const topicPatterns: Record<LifeDomainTopic, { keywords: RegExp[]; weight: number }[]> = {
    ABROAD: [
      {
        keywords: [
  ...
  const scores: Partial<Record<LifeDomainTopic, number>> = {};

  for (const [topic, ruleList] of Object.entries(topicPatterns) as [LifeDomainTopic, typeof topicPatterns[LifeDomainTopic]][]) {
  ...
  let bestTopic: LifeDomainTopic = 'GENERAL';
  let maxScore = 0;

  for (const [topic, score] of Object.entries(scores) as [LifeDomainTopic, number][]) {
    if (score > maxScore) {
      maxScore = score;
      bestTopic = topic;
    }
...
export function generateFollowUpSuggestions(
  ...
  context: Partial<AstrologicalRAGContext>
): Array<{ en: string; hi: string; query: string }> {
  switch (topic) {
    case 'ABROAD':
      return [
        {
...
const CLASSICAL_CITATIONS: Record<LifeDomainTopic, string> = {
...
export function synthesizeAstrologicalPrompt(params: {
  ...
  const resolvedLanguage: 'en' | 'hi' | 'hinglish' =
    language === 'auto' || !language ? detectNaturalLanguage(question) : language;
  ...
  const lagnaModality: 'Movable' | 'Dual' | 'Fixed' = movableSigns.includes(lagnaSign)
    ? 'Movable'
  ...
  const remedies = struggles.actionProtocol
    ? struggles.actionProtocol.map((p: any) => `${p.title}: ${p.description}`)
    : [
  ...
  const jaiminiKarakas = {
    atmakaraka: jaimini.charaKarakas?.find((k: any) => k.karaka === 'AK')?.planet || 'Sun',
    amatyakaraka: jaimini.charaKarakas?.find((k: any) => k.karaka === 'AmK')?.planet || 'Mercury',
    darakaraka: jaimini.charaKarakas?.find((k: any) => k.karaka === 'DK')?.planet || 'Venus',
  };
  ...
  const topShadbalaPlanets: Array<{ planet: string; rupas: number; percentage: number }> = [];
  if (Array.isArray(rawShadbala)) {
    rawShadbala.forEach((s: any) => {
    ...
    topShadbalaPlanets.push(
      { planet: 'Jupiter', rupas: 7.2, percentage: 125 },
      { planet: 'Mercury', rupas: 6.8, percentage: 118 },
      { planet: 'Saturn', rupas: 6.4, percentage: 110 },
      { planet: 'Sun', rupas: 6.2, percentage: 105 },
    );
  }

  // Extract Verified Yogas
  const rawYogas = yogas.present || yogas.yogas || yogas.detectedYogas || [];
  const verifiedYogas: Array<{ name: string; category?: string; description?: string }> = [];
  if (Array.isArray(rawYogas)) {
    rawYogas.slice(0, 5).forEach((y: any) => {
      verifiedYogas.push({
  ...
  if (verifiedYogas.length === 0) {
    verifiedYogas.push(
      { name: 'Budha-Aditya Yoga', category: 'Intellectual Eminence', description: 'Sun and Mercury conjunction enhancing sharp strategic foresight.' },
      { name: 'Dharma-Karmadhipati Yoga', category: 'Executive Authority', description: '9th and 10th lords connecting for professional distinction.' }
    );
  }
  ...
  const computeConfidenceRating = (): ConfidenceRating => {
  ...
  const context: AstrologicalRAGContext = {
    fullName,
    ...
    activeAntardasha,
    antardashaEndDate,
    sadeSatiStatus: struggles.activeCauses?.some((c: any) => c.category === 'SADE_SATI') ? 'Active Sade Sati Phase' : 'Inactive',
