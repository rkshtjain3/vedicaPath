...
import {
  generate12Palaces,
  EARTHLY_BRANCHES,
} from './palace-calculator.js';
import { calculateWuxingJu } from './wuxing-ju-calculator.js';
import { populateStarsInPalaces } from './star-calculator.js';
import {
  calculateSiHuaTransformations,
  attachSiHuaToPalaces,
} from './sihua-calculator.js';

export function evaluateZiWei(chart: BirthChart): ZiWeiChartResult {
  // Use BaZi engine stem-branch calculation as validated Chinese metaphysical provider
  const bazi = evaluateBaZi(chart);

  const yearStem = bazi.fourPillars.year.stem.name as HeavenlyStem;
  const yearBranch = bazi.fourPillars.year.branch.name as EarthlyBranch;
  const hourBranch = bazi.fourPillars.hour.branch.name as EarthlyBranch;
  ...
  const { palaces, mingGongBranchIdx, shenGongBranchIdx } = generate12Palaces(
    lunarMonth,
    hourBranchIdx,
    yearStem
  );
  ...
  const { ziWeiBranchIdx, tianFuBranchIdx } = populateStarsInPalaces(
    palaces,
    lunarDay,
    bureau.number,
    lunarMonth,
