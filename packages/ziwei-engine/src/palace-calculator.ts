...
export const PALACE_TYPES_COUNTER_CLOCKWISE: { type: PalaceType; en: string; cn: string }[] = [
 ...
 */
export function calculateMingGongBranch(lunarMonth: number, hourBranchIndex: number): number {
 ...
 */
export function calculateShenGongBranch(lunarMonth: number, hourBranchIndex: number): number {
  return ((2 + (lunarMonth - 1) + hourBranchIndex) % 12 + 12) % 12;
}
 ...
 */
export function getYinPalaceStemIndex(yearStem: HeavenlyStem): number {
  switch (yearStem) {
    case 'Jia':
    case 'Ji':
      return 2; // Bing (丙)
 ...
 */
export function getPalaceStem(branchIndex: number, yearStem: HeavenlyStem): HeavenlyStem {
 ...
 */
export function generate12Palaces(
  lunarMonth: number,
  hourBranchIndex: number,
  yearStem: HeavenlyStem
): { palaces: ZiWeiPalace[]; mingGongBranchIdx: number; shenGongBranchIdx: number } {
