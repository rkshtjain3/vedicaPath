export function calculateZiWeiBranchIndex(lunarDay, bureauNumber) {
...
export function calculateTianFuBranchIndex(ziWeiBranchIndex) {
...
export function getStarBrightness(starId, branchIndex) {
    // Simplified classical rating mapping
    if ([2, 5, 8, 11].includes(branchIndex))
        return 'Temple';
    if ([0, 3, 6, 9].includes(branchIndex))
        return 'Radiance';
    if ([1, 4, 7, 10].includes(branchIndex))
        return 'Gain';
    return 'Flat';
}
export function populateStarsInPalaces(palaces, lunarDay, bureauNumber, lunarMonth, hourBranchIndex, yearStem, yearBranch) {
    const zwIdx = calculateZiWeiBranchIndex(lunarDay, bureauNumber);
    const tfIdx = calculateTianFuBranchIndex(zwIdx);
    const majorStarPositions = [
    ...
    const auxList = [
        { id: 'lu_cun', nameEn: 'Lu Cun (Treasury)', nameCn: '祿存', category: 'AUXILIARY_BENEFIC', branchIdx: luCunIdx },
        { id: 'qing_yang', nameEn: 'Qing Yang (Goat)', nameCn: '擎羊', category: 'AUXILIARY_MALEFIC', branchIdx: qingYangIdx },
        { id: 'tuo_luo', nameEn: 'Tuo Luo (Humpback)', nameCn: '陀羅', category: 'AUXILIARY_MALEFIC', branchIdx: tuoLuoIdx },
        { id: 'wen_chang', nameEn: 'Wen Chang (Literary)', nameCn: '文昌', category: 'AUXILIARY_BENEFIC', branchIdx: wenChangIdx },
        { id: 'wen_qu', nameEn: 'Wen Qu (Arts)', nameCn: '文曲', category: 'AUXILIARY_BENEFIC', branchIdx: wenQuIdx },
        { id: 'zuo_fu', nameEn: 'Zuo Fu (Left Assistant)', nameCn: '左輔', category: 'AUXILIARY_BENEFIC', branchIdx: zuoFuIdx },
        { id: 'you_bi', nameEn: 'You Bi (Right Assistant)', nameCn: '右弼', category: 'AUXILIARY_BENEFIC', branchIdx: youBiIdx },
    ];
