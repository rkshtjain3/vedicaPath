import {
  ...
  ChartStyle,
  ChartType,
} from '../types/chart-renderer-types.js';
import {
  ...
  RASHI_LORDS,
  formatDegree,
} from '../labels/planet-abbreviations.js';

export function buildChartViewModelFromD1(
  chartData: any,
  style: ChartStyle = 'NORTH_INDIAN'
): ChartViewModel {
...
export function buildChartViewModelFromDivisional(
  divChartData: any,
  ...
  if (divChartData.planets && typeof divChartData.planets === 'object') {
    const planetEntries = Array.isArray(divChartData.planets)
      ...
      : Object.entries(divChartData.planets).map(([name, pos]: [string, any]) => ({
          planet: name,
          ...pos,
        }));
...
export function buildChartViewModel(
  ...
  let baseViewModel: ChartViewModel;
  ...
  if (options?.showTransitOverlay && options?.transitData) {
    ...
    if (Array.isArray(transitPlanets) && transitPlanets.length > 0) {
      ...
      for (const tp of transitPlanets) {
        ...
        const transitChartPlanet: ChartPlanet = {
          planet: `T-${tp.planet}`,
