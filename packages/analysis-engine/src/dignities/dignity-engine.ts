...
import { LordRelationship, PlanetDignity, PlanetFact, PrimaryDignity } from '../types/analysis-types.js';
import { SIGN_LORDS } from '../houses/house-facts.js';

export const EXALTATION: Record<string, { sign: string; degree: number }> = {
...
export const DEBILITATION: Record<string, { sign: string; degree: number }> = {
...
export const MOOLATRIKONA: Record<string, { sign: string; maxDegree: number }> = {
...
export const NAISARGIKA_FRIENDSHIPS: Record<
  string,
  { friends: PlanetName[]; neutral: PlanetName[]; enemies: PlanetName[] }
> = {
...
export function getLordRelationship(planet: PlanetName, signLord: PlanetName): LordRelationship {
  if (planet === signLord) return 'FRIENDLY';
...
export function calculatePlanetDignity(p: PlanetFact): PlanetDignity {
  ...
  let primaryDignity: PrimaryDignity = 'NEUTRAL_SIGN';
  if (rel === 'FRIENDLY') primaryDignity = 'FRIENDLY_SIGN';
  if (rel === 'ENEMY') primaryDignity = 'ENEMY_SIGN';
...
export function calculateAllDignities(planetFacts: PlanetFact[]): PlanetDignity[] {
