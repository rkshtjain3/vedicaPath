...
import { PrintableReportViewModel } from '../types/report-export-types.js';

export function buildPrintableReportViewModel(
  calculationResult: any,
  userProfile?: { fullName?: string }
): PrintableReportViewModel {
  ...
  // 4. Varga Comparison
  const vargaComparisonItems: Array<{
    planet: string;
    d1Sign: string;
    d9Sign: string;
  ...
  const strengthsList: Array<any> = [];
  if (strengthsData.planets) {
    for (const p of strengthsData.planets) {
      const pName = p.planet;
      const shadVal = shadbalaData.planets?.find?.((s: any) => s.planet === pName);
      strengthsList.push({
        planet: pName,
        totalScore: p.score,
  ...
  // 9. Numerology
  const extractNumerologyNumber = (val: any): number | undefined => {
