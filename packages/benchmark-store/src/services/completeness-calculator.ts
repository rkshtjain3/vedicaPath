...
export function calculateReferenceCompleteness(
  referenceValues: Record<string, any> | undefined
): ReferenceCompleteness {
  ...
  let ascScore = 0;
  if (astRef.ascendantLongitude !== undefined || astRef.lagnaLongitude !== undefined) ascScore += 50;
  if (astRef.ascendantSign !== undefined || astRef.lagnaSign !== undefined) ascScore += 50;
  ...
  for (const p of planetNames) {
    ...
    if (lng !== undefined) planetEntered++;
    if (sign !== undefined) planetEntered++;
  }
  ...
  let nakEntered = 0;
  for (const p of planetNames) {
    if (nakRef[p] !== undefined) nakEntered++;
    if (padaRef[p] !== undefined) nakEntered++;
  }
  ...
  let dashaEntered = 0;
  if (dashaRef.birthNakshatra !== undefined) dashaEntered++;
  if (dashaRef.dashaStartingLord !== undefined || dashaRef.mahadashaAtBirth !== undefined) dashaEntered++;
  if (dashaRef.balanceYearsAtBirth !== undefined || dashaRef.balanceAtBirthYears !== undefined) dashaEntered++;
  ...
  let status: ReferenceCoverageStatus = 'NOT_AVAILABLE';
