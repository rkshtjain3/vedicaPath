export function validateReferenceProvenance(provenance) {
    const errors = [];
    if (!provenance) {
        return {
            valid: false,
    ...
    const requestedStatus = provenance.verificationStatus || 'UNVERIFIED';
...
export function detectSelfReferencing(referenceValues, actualChartOutputs, provenance) {
    ...
    if (provenance) {
        ...
        if (entryMethod.includes('INTERNAL_GENERATED') ||
    ...
    for (const pName of planetNames) {
        ...
        const actualP = planets.find((p) => p.planet?.toLowerCase() === pName.toLowerCase());
