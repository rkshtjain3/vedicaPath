...
export class SqliteBenchmarkStore {
    db;
    constructor(dbPathOrInstance = ':memory:') {
        if (typeof dbPathOrInstance === 'string') {
            try {
                ...
                throw new Error(`SqliteBenchmarkStore requires better-sqlite3 native addon: ${err.message}`);
            }
        }
        else {
    ...
    async createCase(caseData) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(newCase.id, newCase.category, newCase.title, newCase.description, newCase.status, newCase.calculationProfileVersion, newCase.createdAt, newCase.updatedAt, JSON.stringify(newCase));
        return newCase;
    }
    async updateCase(id, updates) {
      ...
      WHERE id = ?
    `);
        stmt.run(updatedCase.category, updatedCase.title, updatedCase.description, updatedCase.status, updatedCase.calculationProfileVersion, updatedCase.updatedAt, JSON.stringify(updatedCase), id);
        return updatedCase;
    }
...
//# sourceMappingURL=sqlite-benchmark-store.js.map