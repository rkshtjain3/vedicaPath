...
function getSignLord(signName) {
    ...
    if (s === 'leo')
        return 'Sun';
    if (s === 'sagittarius' || s === 'pisces')
        return 'Jupiter';
    if (s === 'capricorn' || s === 'aquarius')
        return 'Saturn';
    return 'Mars';
}
function getElementOfSign(signName) {
    const s = (signName || '').toLowerCase();
            ...
            sections: [
                {
                    ...
                    highlights: struggles?.activeCauses?.map((c) => `${c.title}: ${c.explanation}`) || [
                        `Active ${currentMaha}-${currentAntar} consolidation cycle`,
                        'Karmic testing designed to forge unshakeable discipline',
                    ],
                    highlightsHi: struggles?.activeCauses?.map((c) => `${c.titleHi || c.title}: ${c.explanationHi || c.explanation}`) || [
                ...
                },
                {
                    ...
                    content: struggles?.actionProtocol
                        ? struggles.actionProtocol.map((p, idx) => `${idx + 1}. **${p.title}:** ${p.description}`).join('\n\n')
                    ...
                    contentHi: struggles?.actionProtocol
                        ? struggles.actionProtocol.map((p, idx) => `${idx + 1}. **${p.titleHi || p.title}:** ${p.descriptionHi || p.description}`).join('\n\n')
...
//# sourceMappingURL=narrative-synthesizer.js.map