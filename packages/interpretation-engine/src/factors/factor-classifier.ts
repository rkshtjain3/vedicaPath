...
export function classifyRuleEvaluation(evalItem: RuleEvaluation): InterpretationFactor {
  let classification: FactorClassification = 'NEUTRAL';
  ...
  for (const eff of evalItem.effects) {
    ...
    if (CHALLENGING_DIMENSIONS.includes(dimUpper)) {
      if (eff.value > 0) hasChallenging = true;
    } else if (SUPPORTIVE_DIMENSIONS.includes(dimUpper)) {
      if (eff.value > 0) hasSupportive = true;
      else if (eff.value < 0) hasChallenging = true;
    }
  }
...
export function classifyDashaActivations(
...
export function classifyTransitEvaluations(
  transits: TransitEvaluation[]
): InterpretationFactor[] {
  const factors: InterpretationFactor[] = [];
  ...
  for (const tr of transits) {
    ...
    let classification: FactorClassification = 'SUPPORTIVE';
    let hasChallenging = false;
