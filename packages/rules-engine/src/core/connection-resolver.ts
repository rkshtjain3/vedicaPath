...
export function isPlanetConnectedToHouse(
  planet: PlanetName,
  house: number,
  analysis: ChartAnalysisResult
): HouseConnectionResult {
  ...
  // 3. Aspects house
  const houseAspects = analysis.aspects.filter(
    (asp) => asp.fromPlanet === planet && asp.toHouse === house
  );
  ...
  const lordAspects = analysis.aspects.filter(
    (asp) => asp.fromPlanet === planet && asp.targetPlanets.includes(houseLord)
  );
  ...
  const lordConjunctions = analysis.conjunctions.filter(
    (conj) =>
      conj.detected &&
