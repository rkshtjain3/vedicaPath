...
export const propRule001: AstrologyRule = {
  id: 'PROP-001',
  domain: 'PROPERTY',
  version: '1.0.0',
...
export const propRule002: AstrologyRule = {
  id: 'PROP-002',
  domain: 'PROPERTY',
  version: '1.0.0',
  evaluate(context: AstrologyRuleContext): RuleEvaluation {
...
export const propRule003: AstrologyRule = {
  id: 'PROP-003',
  domain: 'PROPERTY',
  version: '1.0.0',
  evaluate(context: AstrologyRuleContext): RuleEvaluation {
...
export const propRule004: AstrologyRule = {
  id: 'PROP-004',
  domain: 'PROPERTY',
  version: '1.0.0',
  evaluate(context: AstrologyRuleContext): RuleEvaluation {
    const trikLords = [6, 8, 12].map(
      (h) => context.analysis.houseLordFacts.find((hl) => hl.house === h)?.lord
    ).filter(Boolean);
    ...
    for (const trikLord of trikLords) {
      ...
      const aspectingHouse4 = context.analysis.aspects.filter(
        (asp) => asp.fromPlanet === trikLord && asp.toHouse === 4
      );
      ...
      if (house4Lord) {
        const aspectingLord4 = context.analysis.aspects.filter(
          (asp) => asp.fromPlanet === trikLord && asp.targetPlanets.includes(house4Lord)
        );
...
export const propertyRulesList: AstrologyRule[] = [
  propRule001,
  propRule002,
  propRule003,
  propRule004,
