import { MuhurthaWindows, TimeInterval } from '../types/panchanga-types.js';
...
function createInterval(startMins: number, endMins: number): TimeInterval {
  return {
...
const RAHU_KALAM_PARTS: Record<number, number> = {
...
const YAMAGANDA_PARTS: Record<number, number> = {
...
const GULIKA_PARTS: Record<number, number> = {
...
export function calculateMuhurthaWindows(
  dayIndex: number,
  sunriseMinutes: number = 360, // 6:00 AM
  sunsetMinutes: number = 1080, // 6:00 PM
  isAstronomical: boolean = false
): MuhurthaWindows {
