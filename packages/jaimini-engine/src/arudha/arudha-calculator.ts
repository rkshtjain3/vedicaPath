...
import { ArudhaEvidence, ArudhaPada } from '../types/jaimini-types.js';

interface PadaMetadata {
  houseNumber: number;
  code: string;
  name: string;
  nameHi: string;
...
const PADA_METADATA: PadaMetadata[] = [
  {
    houseNumber: 1,
    code: 'AL',
 ...
 */
export function calculateArudhaPadas(birthChart: BirthChart): ArudhaPada[] {
  const d1LagnaSignId = (birthChart as any).lagna?.sign?.id ?? (birthChart as any).ascendant?.rashi?.id ?? 1;

  const getPlanetSignId = (planet: PlanetName): number => {
  ...
  for (let h = 1; h <= 12; h++) {
    ...
    let exceptionRule: string | undefined;
