...
export interface UTCInstant {
  ...
  minute: number; // 0-59
  second: number; // 0-59
  decimalHour: number; // UTC hour in decimal format
  isoString: string; // ISO 8601 string in UTC
}
...
function dateToUTCInstant(d: Date): UTCInstant {
...
export function resolveLocalTime(
  ...
  occurrencePref: 'FIRST' | 'SECOND' = 'FIRST'
): LocalTimeResolution {
 ...
 */
export function getUTCInstant(input: BirthTimeInput): UTCInstant {
